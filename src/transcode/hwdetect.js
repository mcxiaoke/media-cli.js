 ...
 *   与第二层「文件级探测」（src/transcode/hwaccel.js 的 probeLayer）配合：
 ...
 * @param {AbortSignal} [opts.signal] 取消硬件探测的信号
...
export async function detectHardwareCapabilities({
    ...
    if (signal?.aborted) {
        const error = new Error("hardware probe cancelled")
...
export function candidateTiers(caps, { decodeMode = "auto", hwaccel } = {}) {
    ...
    // P1-1：d3d 是各厂商共用的 Windows「解码兜底层」，本机实测比 swdec（CPU 解码 + 硬件编码）
    // 还慢约 38%（RTX 4070：d3d≈892ms vs swdec≈648ms，见 temp/review / FFMPEG-CMD-REVIEW-20260921.md），
    // 故把它从「厂商解码层之后」延后到「swdec 之后」，仅在主解码层全部失败时才兜底。
    const isChainTierAvail = (t) => {
    ...
    const list = []
    // 1) 主厂商硬件解码层（cuda / qsv / amf）：d3d 兜底层在下方独立处理
    for (const t of allowed) {
        if (t === "d3d") continue
        if (isChainTierAvail(t)) list.push(t)
    }
    // 2) swdec 层（CPU 解码 + CPU scale + 硬件编码，T7 新增）：
    //    位置固定在「主厂商解码层之后、d3d 兜底与 cpu 之前」—— 它不提升解码能力，
    //    只改善"解码层被拦下后原本会落到 libx264/libx265"的那一段。
    //    入场条件：本机存在该厂商的硬件编码器（与解码能力无关）。
    if (allowed.includes("d3d") && swdecUsable(caps, vendor)) {
        list.push("swdec")
    }
    // 3) d3d 解码兜底层（Windows 通用，延后到 swdec 之后）
    if (allowed.includes("d3d") && isChainTierAvail("d3d")) {
        list.push("d3d")
    }
    // 4) 无 d3d 的厂商（理论上不存在）或 swdec 未入场时，保证 swdec 仍在 cpu 之前补上
    if (!list.includes("swdec") && swdecUsable(caps, vendor)) {
        list.push("swdec")
    }
    list.push("cpu")
    return list

...
| ✅ 已修复（第一轮） | 19 | 见第二节 |
| ✅ 已修复（第二轮） | 14 | 见第三节 |
| ✅ 已修复（第三轮） | 4 | T-12、T-16、T-18、T-20，见第四节 |
| ⏸️ 保留搁置 | 2 | T-13（无 24fps 素材）、T-15（环境绑定） |
| ⚪ 经核实不改 | 2 | T-4（用户明确说明无 EXIF 场景）、T-10（语义自洽，非缺陷） |

**当前状态：21 项待修复问题中，19 项已修复，2 项搁置，2 项经核实无需修改。**

测试门禁：`npm test` 从"3 文件 28 用例"提升为"全量 43 用例"，**43/43 通过**。
...
| # | 位置 | 问题 | 修复与验证 |
| - | ---- | ---- | ---------- |
| T-12 | `lib/ffmpeg_presets.js` | `vc`/`vcodec` 被映射为 `videoCopy`、`ac`/`acodec` 映射为 `audioCopy`，与注释声明的 codec 语义相反 → `--ffargs "vc=h264"` 静默得到 `-c:v copy`（流复制），产出错误视频 | 别名改映射为 `videoCodec`/`audioCodec`；新增 `replaceCodecInArgs()` 替换参数串中的 `-c:v`/`-c:a` 值（无则前置）。实测 `vc=h264_nvenc` 正确产出编码器参数，不再含 copy |
| T-16 | `lib/unicode.js` | 三处正则缺陷导致换行、`\|`、`/`、`}` 被判为"中文/日文/韩文" | ① `REGEX_CHINESE_ALL` 缺分组括号（`^A\|B\|C$` 被解析为 `(^A)\|B\|(C$)`）→ 改为单字符类 + 锚点；② `REGEX_CHINESE_ONLY` 模板串含字面换行 → 合并为单行；③ `REGEX_ONLY_HANGUL` 的 `\\u00a1` 双反斜杠与前面的 `.` 组成范围 `.-\` → 改为单反斜杠；④ `REGEX_HAS_HIRA_OR_KANA` 字符类内多余 `}` → 移除。**12 项行为验证全部通过** |
| T-18 | `lib/arg_parser.js` | ① 用 `/;\|:\|#/` 直接切分，值含冒号（`size=16:9`、时间、URL）被误切抛错；② `required` 只检查 `undefined`，`q=`（解析为 null）可绕过 | ① 新增 `splitKeyValuePairs()`：仅当分隔符后确实跟着新的 `key=value` 时才切分，`size=16:9` 与 `a=1:b=2` 均正确；② required 把 null/空串一并视为未提供 |
| T-20 | `package.json`、`test/*` | `npm test` 只跑 3 个白名单文件，排除了 2 个失败文件，制造"全绿"假象 | ① 重写 `test_remove_command.js`（真实 yargs 实例 + stub `inquirer.prompt` + 行为断言）：**1/7 → 9/9**；② 重写 `test_decode_command.js`（标准 `describe/it` + `process.execPath` + 固定 cwd + GBK 往返回归）：**0/1 → 6/6**；③ `test` 脚本改为 `node --test "test/*.js"`：**28 → 43 用例，43/43 通过** |

### 4.1 第三轮验证证据

```
npm run check   →  全部 *.js 通过
npm test        →  43/43 通过（全量，无排除文件）

T-12  vc=h264_nvenc → videoArgs 含 h264_nvenc，不含 copy        ✅
T-16  strOnlyChinese("ab\ncd") / ("a|b") / ("abc/") 均为 false  ✅
      strOnlyChinese("中文") / strOnlyHangul("한글") 均为 true   ✅
T-18  parseArgs("size=16:9") → {size:"16:9"}（此前抛错）        ✅
      parseArgs("a=1:b=2")   → {a:1,b:2}（保持可用）            ✅
T-20  test_remove_command 9/9、test_decode_command 6/6          ✅
```

---

## 五、待修复问题列表（原始 21 项，含处置状态）

> 本节保留原始编号便于追溯。已处理的项标 ✅，未处理的标注原因。

按模块分组，每项标注证据来源与修复建议。**优先级**：🔴 高（功能失效或数据风险）／🟡 中（行为异常）／⚪ 低（质量与一致性）。

### 5.1 `pick` 命令（照片智能挑选）—— 🔴 两条去重路径全部失效
...
**T-12 ✅已修复（第三轮）🟡 `vc` / `ac` 别名映射为 `videoCopy` / `audioCopy`（语义可疑）**
...
**T-13 ⏸️保留搁置（无 24fps 素材）🟡 `hevc_speed` 预设帧率逻辑可能生成 `fps=0`**
...
**T-16 ✅已修复（第三轮）🟡 `unicode.js` 判定函数污染：换行、`|`、`/`、`}` 被判为"中文/日文/韩文"**
...
**T-18 ✅已修复（第三轮）⚪ `arg_parser` 冒号切分与 required 校验**
...
**T-20 ✅已修复（第三轮）🔴 `npm test` 刻意排除两个失败测试文件，门禁形同虚设**
...
以下问题确实存在且已核实，但**改动大、影响面广、或需要你先做设计决策**，暂未处理。

| # | 问题 | 状态 | 说明 |
| - | ---- | ---- | ---- |
| S-1 | T-5 并发重命名 TOCTOU | ✅ **已解决** | 原搁置理由是"需改为串行、牺牲性能"；后找到**两阶段重命名**方案（先改临时名清空占位、再改最终名），并发性能不变，已在第二轮修复 |
| S-2 | T-16 `unicode.js` 判定函数污染 | ✅ **已解决** | 第三轮已修复（见第四节），并做了 12 项行为验证 |
| S-3 | T-12 `vc` / `ac` 别名语义 | ✅ **已解决** | 用户确认 `vc`=codec、`ac`=codec，第三轮已修复 |
| S-4 | T-15 `scale_cuda` / `libfdk_aac` 硬编码 | ⏸️ **仍搁置** | 属环境绑定决策。你本机是 N 卡 + 自建 ffmpeg，改默认值会影响日常使用。**若要做需要你确认目标环境**（是否要 CPU 降级路径） |
| S-5 | T-18 `arg_parser` 分隔符 | ✅ **已解决** | 第三轮采用"仅当分隔符后跟着新 key=value 时才切分"方案，既支持 `size=16:9`，又保持 `a=1:b=2` 可用，不破坏既有写法 |
| S-6 | T-20 测试门禁 | ✅ **已解决** | 用户授权直接修，第三轮完成：两个测试文件修复，`npm test` 改为全量 43 用例 |

**仍有 2 项大改动未纳入**（两份报告均有提及，但超出"改动小收益大"范围）：

- **`cmd_pick` 感知哈希去重整体重写**：T-1/T-2 已修（功能恢复），但 `cmd_pick.js` 与 `lib/image_hash.js` 仍有**两套几乎同构的去重实现**，且去重是 O(n²) 未桶化（5 万张 ≈ 12.5 亿次 hammingDistance）。建议后续收敛为单一实现并做分桶优化。

...
> 版本: 2026-09-22依据: 项目自有文档
> `docs/ffmpeg/`（含官方文档镜像、编码器参数指南、滤镜指南、硬件加速指南等）+ 项目现有代码 (`cmd/cmd_ffmpeg.js`,
> `lib/ffmpeg_build.js`, `lib/ffmpeg_presets.js`, `lib/hwaccel.js`,
> `lib/arg_parser.js`) 目标: 在不破坏 auto 降级逻辑、不新增独立抽象层的前提下，让 CLI 自定义参数可靠、位置可控、与硬件分层兼容。注意：`temp/ffmpeg-docs/`
> 目录在当前工作区不存在，本文档以 `docs/ffmpeg/` 下已引用的官方内容为准。

---

## 1. FFmpeg 命令行参数分类与位置规则

FFmpeg 命令行参数按**位置**分为 4 大类，顺序不可随意调换：
...
| 分类                | 位置                             | 典型参数                                                                                    | 当前支持                           | 问题                                                         |
| ------------------- | -------------------------------- | ------------------------------------------------------------------------------------------- | ---------------------------------- | ------------------------------------------------------------ |
| **全局选项**        | 命令最前                         | `-y`, `-nostats`, `-benchmark`, `-hide_banner`, `-loglevel`                                 | ❌ 无 CLI 入口                     | 用户无法覆盖日志级别或关闭 banner                            |
| **输入选项**        | `-i` 之前                        | `-ss`, `-t`, `-to`, `-f`, `-r` (输入), `-hwaccel`, `-hwaccel_device`, `-itsoffset`, `-loop` | ❌ 无 CLI 入口                     | 无法 seek/trim、无法调整音画同步                             |
| **输出选项-流级**   | 输出文件前，作用于特定流         | `-c:v`, `-c:a`, `-c:s`, `-b:v`, `-crf`, `-preset`, `-tune`, `-g`, `-bf`, `-map`             | ⚠️ 部分受限                        | `--ffargs` 只支持白名单；`--video-args` 含 `-c:v` 时整段丢弃 |
...
| **音频专属**        | 输出文件前                       | `-ar`, `-ac`, `-vol`, `-af`                                                                 | ❌ 无 CLI 入口                     | 无法强制采样率、声道数、音量                                 |
| **字幕专属**        | 输出文件前                       | `-c:s`, `-sn`, `-fix_sub_duration`, `-canvas_size`                                          | ❌ 无 CLI 入口                     | 字幕流默认被丢弃或重编码                                     |

### 1.2 关键位置规则（官方文档）

- **`-ss` 位置决定精度**：放在 `-i`
  前 = 快速 seek（可能非精确帧）；放在输出文件前 = 精确到帧（解码并丢弃）。
- **`-t`/`-to` 位置**：放在 `-i`
  前 = 限制输入读取时长；放在输出文件前 = 限制输出写入时长。两者可叠加使用（如
  `-ss 10 -i in.mp4 -t 30 out.mp4` = 从 10 秒开始取 30 秒）。
- **`-hwaccel` 是输入侧选项**：必须放在 `-i` 之前。当前 `buildHwaccelArgs` 已正确处理，将其放入
  `inputArgs`。
- **`-c:v`/`-c:a`/`-c:s` 是输出侧选项**：放在输出文件之前。当前 `buildEncoderArgs` 将其放入
  `middleArgs`（视频/音频参数块）。
- **`-metadata` 是输出侧选项**：放在输出文件之前。当前 `buildMetaArgs` 将其放入 `middleArgs`。
...
4. **位置可控**：新增参数带位置标记，明确落在 input 前 / output 前 / 编码器块内。
...
| `@filter` | `-vf`/`-af` 内         | `middleArgs`（滤镜链中）           | 追加到 pre/post/scale 段      |
| `@global` | 命令最前               | 最前                               | `-y`、`-nostats`、`-loglevel` |

### 3.3 编码器参数 tier 差异化语法
...
### 3.4 与现有 `--ffargs` 的共存策略

| 参数形式                    | 处理方式                       | 优先级           |
| --------------------------- | ------------------------------ | ---------------- |
| `--ffargs "vb=2000,vq=22"`  | 走现有 `applyFfargs` 白名单    | 与白名单参数同级 |
| `--arg "@input -ss 10"`     | 走新位置标记体系               | 最高（明确位置） |
| `--arg "@video -tune film"` | 追加到编码器参数块             | 最高（明确位置） |
| `--video-args "-tune film"` | 走现有机制（附加到 videoArgs） | 中（位置不明确） |

**冲突解决**：同一参数若同时通过 `--ffargs` 和 `--arg` 指定，`--arg` 优先（因其位置明确）。
...
```javascript
// cmd/cmd_ffmpeg.js yargs builder
.option("arg", {
    alias: "a",
    type: "array",  // 支持多次使用
...
| 现有选项       | 增强方式                  | 说明                                                                       |
| -------------- | ------------------------- | -------------------------------------------------------------------------- |
| `--ffargs`     | 白名单扩展 + 非白名单警告 | 保留现有 21 个白名单；未知键不再静默丢弃，而是 warn 提示并建议改用 `--arg` |
...
| `lib/ffmpeg_presets.js` | `createFromArgv`            | 解析 `--arg` 数组，生成 `userArgs.argOptions`                      |
...
finalArgs.push(...(argOptions.output || []))
...
| `--video-bitrate 2000`         | 不变，直接覆盖 preset.videoBitrate                 |
| 新用法 `--arg "@input -ss 10"` | 新能力，不影响旧用法                               |
...
```bash
# 截取片段（从 10 秒开始，取 30 秒）
mediac ffmpeg input --arg "@input -ss 10 -t 30" --preset hevc_2k

# 强制输出元数据
mediac ffmpeg input --arg "@output -metadata title='Vacation 2024'" --preset hevc_2k

# 强制音频采样率和声道
mediac ffmpeg input --arg "@audio -ar 48000 -ac 2" --preset hevc_2k

# 追加反交错滤镜（不丢失缩放）
mediac ffmpeg input --arg "@filter pre=yadif" --preset hevc_2k
```

### 8.2 高级调优

```bash
# 按 tier 差异化：NVENC 开 AQ，QSV 开 lookahead，CPU 用 slow
mediac ffmpeg input \
  --arg "@video[cuda] -spatial_aq 1 -temporal_aq 1" \
  --arg "@video[qsv] -look_ahead 1 -async_depth 4" \
  --arg "@video[cpu] -preset slow -tune film" \
  --preset hevc_2k
...
```bash
# --ffargs 调白名单参数 + --arg 调位置参数
mediac ffmpeg input \
  --ffargs "vq=22,vb=4000" \
  --arg "@input -ss 10" \
  --arg "@video -tune film" \
  --preset hevc_2k
```
...
| 5   | `--arg` 与 `--ffargs` 同时指定同一参数（如 `--ffargs vq=22` + `--arg "@video -crf 22"`）时优先级？ | `--arg` 优先，因其位置明确；warn 提示冲突                                              |

---

## 10. 附录：FFmpeg 参数位置速查表

...
export async function walk(root, options = {}) {
  ...
  log.info(
    logTag,
    `total ${files.length} files after filter in ${humanTime(initMs)}.`
  )
  return files
}
...
export async function getDirectorySize(directory, concurrency = 8) {
  ...
  async function processQueue() {
    while (queue.length > 0) {
      ...
      try {
        const files = await fs.readdir(currentDir)
        const fileStats = await Promise.all(files.map(file => fs.stat(path.join(currentDir, file))))
  ...
  async function processQueue() {
    while (queue.length > 0) {
      ...
      try {
        ...
        await Promise.all(fileStats.map(async (stats) => {
          if (stats.isDirectory()) {

...
import { humanTime, isMediaFile, pathShort } from "./helper.js"
 ...
 * @returns {Promise<Array>} 包含文件条目的数组，每个条目包含路径、统计信息等
 */
export async function walk(root, options = {}) {
    ...
    // 控制进度条更新频率，每2秒最多更新一次（每次遍历独立计时，避免跨调用共享状态）
...
const FILELIST_TAG = "filelist"

/**
 * 解析文本文件清单（filelist），逐行读取媒体文件路径。
 *
 * 一行一个文件；空行与 `#`/`//` 开头的注释行跳过。仅保留「存在且是媒体文件」的条目，
 * 缺失/非文件/非媒体类型的路径记日志并跳过。条目结构与 `walk` 完全一致，
 * 保证调用方无需区分来源即可继续处理。
 *
 * @param {string} filelistPath - 清单文件路径（已 resolve，需保证存在）
 * @param {string} root - 兜底 root（tree 输出模式用）
 * @returns {Promise<Object[]>} 媒体文件条目数组
 */
export async function parseFilelist(filelistPath, root) {
    const text = await fs.readFile(filelistPath, "utf8")
    const lines = text
        .split(/\r?\n/)
        .map((l) => l.trim())
        .filter((l) => l.length > 0 && !l.startsWith("#") && !l.startsWith("//"))
    log.logInfo(FILELIST_TAG, `Filelist: ${lines.length} entries from ${pathShort(filelistPath)}`)
    const entries = []
    for (const line of lines) {
        ...
        try {
            ...
            if (st.isFile()) {
                if (!isMediaFile(name)) {
                    log.logDebug(FILELIST_TAG, `Skip[NotMedia] (filelist): ${p}`)
                    continue
                }
                entries.push({
                    root, // tree 输出模式兜底；filelist 模式下通常用不上
                    name,
                    path: p,
                    stats: st,
                    ctime: st.ctime || 0,
                ...
                log.logWarn(FILELIST_TAG, `Skip[NotFile] (filelist): ${p}`)
            }
        } catch (err) {
            log.logWarn(FILELIST_TAG, `Skip[Missing] (filelist): ${p} (${err.message})`)
        }
    }
    log.logInfo(FILELIST_TAG, `Filelist: ${entries.length} files kept`)
    return entries
}
...
export async function getDirectorySize(directory, concurrency = 8) {
    ...
    // 统计"进行中"的 worker 数。旧实现用 `while (queue.length > 0)` 判断退出，
    // 队列在某个瞬间为空时 worker 会提前退出，不再等待其他 worker 入队，
    // 导致 concurrency 参数几乎无效、深层目录可能被漏算。
    //
    // 正确收敛条件：队列为空 且 没有任何 worker 正在处理目录
    //（pending === 0）。若仍有 worker 在处理，它可能马上入队新子目录。
    let pending = 0
    ...
    async function processQueue() {
        while (true) {
            ...
            if (currentPath === undefined) {
                // 队列暂时为空：若仍有 worker 在处理目录，等待其可能入队的子目录
                if (pending === 0) break
            ...
            try {
                ...
                await handleError(error, { operation: "getDirectorySize", dir: currentPath })
            } finally {
                pending--
            }
        }
    ...
    const workers = Array.from({ length: workerCount }, processQueue)
...
export async function getDirectoryFileCount(directoryPath) {
    try {
        ...
        async function traverseDirectory(currentPath) {
            ...
            for (const file of files) {
                const filePath = path.join(currentPath, file)
                // 必须用 lstat 而非 stat：stat 会跟随符号链接，
                // 一旦目录树里出现指向祖先目录的链接（含 Windows junction），
                // 递归就会成环直到栈溢出。lstat 下链接不是目录，自然被跳过。
                const stats = await fs.lstat(filePath)
 ...
 * 跨卷安全的移动
 *
 * `fs.rename` 只在同一卷内可用：源与目标不同盘（`--output` 指向另一个盘时必现）
 * 会抛 `EXDEV: cross-device link not permitted`，命令直接失败。
 * 这里在 EXDEV 时退化为「复制 + 删除源」，语义与 rename 保持一致（可覆盖目标）。
 *
 * @param {string} src - 源路径（文件或目录）
 * @param {string} dst - 目标路径
 * @returns {Promise<string>} 目标路径
 */
export async function moveSafe(src, dst) {
    try {
        ...
        log.info(
            "moveSafe",
            `EXDEV, falling back to copy+remove: ${pathShort(src)} => ${pathShort(dst)}`,
        )

/*
 * 文件: i18n.js
 * 创建: 2026-02-12 09:55:00 +0800
 * 作者: mcxiaoke (github@mcxiaoke.com)
 * 许可证: Apache License 2.0
 *
 * 国际化(i18n)系统 - 提供多语言支持
 * 支持中英文双语切换，自动检测系统语言环境
 */
...
// 翻译资源数据库 - 每个键包含中英文翻译
// 格式: "key": { zh: "中文翻译", en: "English translation" }
const translations = {
    // ========== 通用消息 ==========
    ...
    "option.common.keepRoot": {
        zh: "保留输出文件的根目录结构",
        en: "Keep the root directory structure in output",
    },
    "option.common.keepMetadata": {
        zh: "确保保留输出文件的元数据",
        en: "Ensure to keep the metadata in output",
    },
    ...
    "option.common.autoConfirm": {
        zh: "自动确认所有交互提示，跳过 y/N 询问（自动化测试用）",
        en: "auto-confirm all interactive prompts, skip y/N asks (for automation)",
    },
    ...
    "option.common.yes": {
        zh: "自动确认所有操作，跳过用户交互",
        en: "Auto confirm all operations, skip user prompts",
    },
    ...
    "commands.lrmove.nothing.to.do": {
        zh: "没有找到 JPEG 文件夹，无需操作。",
        en: "No JPEG folders found, nothing to do.",
    },
    "commands.lrmove.skip.exists": {
        zh: "跳过已存在的: {{dst}}",
        en: "Skipped existing: {{dst}}",
    },
    ...
    "compress.delete.confirm": {
        zh: "确认删除 {{count}} 个已压缩的源文件？",
        en: "Are you sure to DELETE {{count}} source files that were compressed?",
    },
    ...
    "compress.force": {
        zh: "禁用文件名过滤规则，强制处理所有文件",
        en: "Disable filename filtering rules, force compress all files",
    },
    ...
    "dcim.option.log": {
        zh: "将操作日志导出到指定文件",
        en: "Export operation log to specified file",
    },
    "dcim.option.backupDir": {
        zh: "将原始文件备份到指定目录",
        en: "Backup original files to specified directory",
    },
    "dcim.option.checkDate": {
        zh: "检查文件日期，跳过凌晨时段(0-7点)的文件",
        en: "Check file date, skip files created in early morning (0-7am)",
    },
    ...
    "ffmpeg.test.sample": {
        zh: "试运行(TestMode)下任务数较多({{total}})，仅抽样预览 {{count}} 个(每 {{step}} 个取 1)，不会全部列出",
        en: "TestMode with many tasks ({{total}}), previewing only {{count}} sampled (1 of every {{step}}), not all will be shown",
    },
    "ffmpeg.test.mode.skip": {
        zh: "[TestMode] 未实际执行转码",
        en: "[TestMode] ffmpeg not actually executed",
    },
    ...
    "ffmpeg.speed": {
        zh: "改变视频和音频的速度（倍率 0.5–2.0，0=不变速；音视频同步变速）",
        en: "Change video & audio speed (rate 0.5–2.0, 0=none; A/V stay in sync)",
    },
    "ffmpeg.video.args": {
        zh: "追加视频编码参数（拼到编码器参数块末尾，后写覆盖；勿含 -c:v，换编码器请用 --video-codec）",
        en: "Append video encoder args (added after the encoder block, later wins; no -c:v — use --video-codec to change encoder)",
    },
    "ffmpeg.video.bitrate": {
        zh: "设置视频目标码率（纯数字=bps，或带 k/m/g 单位如 233k、3M）",
        en: "Set video target bitrate (bare number=bps, or with k/m/g suffix e.g. 233k, 3M)",
    },
    "ffmpeg.video.copy": {
        zh: "复制视频流到输出，不重新编码",
    ...
    "ffmpeg.video.codec": {
        zh: "显式指定视频编码器（如 h264_nvenc / libx264 / copy）",
        en: "Force video encoder (e.g. h264_nvenc / libx264 / copy)",
    },
    ...
    "ffmpeg.audio.args": {
        zh: "追加音频编码参数（拼到音频参数块末尾，后写覆盖；不影响内置 copy/降级判断）",
        en: "Append audio encoder args (added after the audio block, later wins; keeps built-in copy/fallback logic)",
    },
    "ffmpeg.audio.bitrate": {
        zh: "设置音频目标码率（纯数字=bps，或带 k/m/g 单位如 128k、3M）",
        en: "Set audio target bitrate (bare number=bps, or with k/m/g suffix e.g. 128k)",
    },
    "ffmpeg.audio.copy": {
        zh: "复制音频流到输出，不重新编码",
    ...
    "ffmpeg.filters": {
        zh: "追加视频滤镜（拼到 -vf 链末尾/缩放后，保留预设的缩放滤镜）",
        en: "Append video filters (added to the end of -vf chain, keeps preset scale filter)",
    },
    "ffmpeg.metadata": {
        zh: '输出元数据，多组用 ; 分隔，值可含空格：--metadata "title=My Video;comment=hi"',
        en: "Output metadata, ';'-separated key=value, values may contain spaces: --metadata \"title=My Video;comment=hi\"",
    },
    "ffmpeg.error.videoArgsCodec": {
        zh: '--video-args 不允许包含 -c:v：编码器由预设/硬件分层决定，请改用 --video-codec 或 --ffargs "vc=..."',
        en: '--video-args must not contain -c:v: the encoder is chosen by preset/hardware tier; use --video-codec or --ffargs "vc=..." instead',
    },
    "ffmpeg.warn.encoderSpecific": {
        zh: "检测到编码器专属参数 {{tokens}}：在 auto 分层下，其它编码器（NVENC/QSV/AMF/x264）可能不识别，导致真实编码报错或被重试降级到 CPU",
        en: "Encoder-specific arg(s) {{tokens}} detected: under auto tiering other encoders (NVENC/QSV/AMF/x264) may reject them, causing encode errors or CPU fallback via retry",
    },
    ...
    "ffmpeg.strict": {
        zh: "严格模式：禁用所有自动降级（硬件层回退 CPU、编码器降级、失败重试、10bit 软解规避），探测到软硬件不支持时跳过该文件",
        en: "strict mode: disable all fallbacks (hw tier->cpu, encoder downgrade, retry, 10bit sw decode); skip files whose hw/sw is unsupported",
    },
    "ffmpeg.strict.hw": {
        zh: "严格模式：所有硬件层均不可用于该输入，已跳过该文件（未转码）。请用 --decode-mode cpu 显式指定软解",
        en: "strict mode: no hw tier usable for this input, file skipped (not transcoded). Use --decode-mode cpu explicitly",
    },
    "ffmpeg.strict.audio": {
        zh: "严格模式：音频编码器 {{codec}} 在当前 ffmpeg 构建中不可用，该文件已跳过（未转码、未降级）",
        en: "strict mode: audio encoder {{codec}} unavailable in this ffmpeg build, file skipped (not transcoded, no fallback)",
    },
    "ffmpeg.strict.retry": {
        zh: "严格模式：转码失败，跳过 CPU 降级重试",
        en: "strict mode: transcode failed, skipping CPU fallback retry",
    },
    "ffmpeg.strict.10bit": {
        zh: "严格模式：H264 10bit 无法硬件解码，该文件已跳过（未自动切换软解）",
        en: "strict mode: H264 10bit cannot be hw-decoded, file skipped (no automatic software decode)",
    },
    "ffmpeg.strict.skip.count": {
        zh: "严格模式：共跳过 {{count}} 个软硬件不支持的文件（详见日志）",
        en: "strict mode: skipped {{count}} file(s) with unsupported hw/sw (see log for details)",
    },
    "ffmpeg.skip.count": {
        zh: "共跳过 {{count}} 个文件（目标已存在或格式无效，详见日志）",
        en: "Skipped {{count}} file(s) (destination exists or invalid format, see log for details)",
    },
    ...
    "ffmpeg.error.preset": {
        zh: "未使用 --preset 参数指定有效的预设",
        en: "Invalid preset args, use --preset to specify a valid preset",
    },
    "ffmpeg.error.jobs": {
        zh: "jobs 必须为正数",
        en: "Jobs must be a positive number",
    },
    "ffmpeg.error.speed": {
        zh: "速度倍率必须在 0.5 到 2.0 之间（0 表示不变速）",
        en: "Speed must be between 0.5 and 2.0 (0 = no change)",
    },
    "ffmpeg.error.dimension": {
        zh: "dimension 必须为非负数",
        en: "Dimension must be non-negative",
    },
    "ffmpeg.error.invalidInput": {
        zh: "无效输入: {{path}}",
        en: "Invalid Input: {{path}}",
    },
    "ffmpeg.error.filelist": {
        zh: "文件清单不存在: {{path}}",
        en: "File list not found: {{path}}",
    },
    "ffmpeg.filelist": {
        zh: "从文本文件读取要处理的媒体文件路径（一行一个，支持 # 注释）；命中时跳过目录遍历",
        en: "Read media file paths from a text file (one per line, supports # comments); skips directory walk",
    },
    "ffmpeg.error.unknownOutputMode": {
        zh: "未知的输出模式: {{mode}}",
        en: "Unknown output mode: {{mode}}",
    },
    ...
    "remove.files.skipped": {
        zh: "跳过 {{count}} 个文件（不满足删除条件）",
        en: "{{count}} files skipped (do not match remove conditions)",
    },
    "remove.nothing.to.do": {
        zh: "没有满足条件的文件，无需操作。",
        en: "No files match the conditions, nothing to do.",
    },
    ...
    "remove.sizel": {
        zh: "大小不小于此值的文件将被删除（单位:K，1024 进制，含边界）",
        en: "Files size >= value will be removed (unit:K, 1024-based, inclusive)",
    },
    "remove.sizer": {
        zh: "大小不大于此值的文件将被删除（单位:K，1024 进制，含边界）",
        en: "Files size <= value will be removed (unit:K, 1024-based, inclusive)",
    },
    "remove.pattern": {
        zh: "文件名模式匹配此值的文件将被删除",
    ...
    "remove.audio": { zh: "根据音频元数据删除文件", en: "Remove files by audio metadata" },
    "remove.type": {
    ...
    "remove.mtime": {
        zh: "基于文件修改时间筛选 (格式: 1d=天内, 1w=周内, 1m=月内, 1y=年内)",
        en: "Filter by file modified time (format: 1d=days, 1w=weeks, 1m=months, 1y=years)",
    },
    "remove.ctime": {
        zh: "基于文件创建时间筛选 (格式: 1d=天内, 1w=周内, 1m=月内, 1y=年内)",
        en: "Filter by file creation time (format: 1d=days, 1w=weeks, 1m=months, 1y=years)",
    },

    // ========== decode 命令 ==========
    "decode.description": {
        zh: "识别和处理文件由于编码导致的乱码字符",
        en: "Recognize and process messy characters caused by file encoding",
    },
    "decode.text.input.required": { zh: "需要文本输入", en: "text input required" },
    "decode.positional.strings": {
        zh: "需要解码的字符串",
        en: "strings to decode",
    },
    "decode.option.fromEnc": {
        zh: "源编码，指定从哪种编码开始尝试解码",
        en: "source encoding, specify which encoding to start decoding from",
    },
    "decode.option.toEnc": {
        zh: "目标编码，指定解码后转换到哪种编码",
        en: "target encoding, specify which encoding to convert to after decoding",
    },
    "decode.option.files": {
        zh: "需要解码的文件路径，可以使用通配符",
        en: "file paths to decode, can use wildcards",
    },
    "decode.option.recursive": {
        zh: "递归处理目录中的文件",
        en: "process files in directories recursively",
    },
    "decode.tryDecoding": {
        zh: "尝试解码",
        en: "Try Decoding",
    ...
    "decode.no.files.found": {
        zh: "未找到匹配模式的文件",
        en: "No files found for pattern",
    },
    "decode.found.files": {
        zh: "找到 {{count}} 个要处理的文件",
        en: "Found {{count}} files to process",
    },
    "decode.processing.files": {
        zh: "处理文件中",
        en: "Processing files",
    },
    "decode.processing.file": {
        zh: "处理文件",
        en: "Processing file",
    },
    "decode.file.content.length": {
        zh: "文件内容长度",
        en: "File content length",
    },
    "decode.result": {
        zh: "结果",
        en: "Result",
    ...
    "decode.badUnicode": {
        zh: "不良Unicode字符",
        en: "bad Unicode chars",
    },
    "decode.error.file": {
        zh: "处理文件错误 {{path}}",
        en: "Error processing file {{path}}",
    },
    "decode.processed.out.of": {
        zh: "已处理 {{processed}}/{{total}} 个文件",
        en: "Processed {{processed}} out of {{total}} files",
    },
    "decode.file": {
        zh: "文件",
        en: "FILE",
    ...
    "decode.example.decode.string": {
        zh: "解码单个乱码字符串",
        en: "Decode a single messy string",
    },
    "decode.example.decode.files": {
        zh: "解码当前目录下的所有 txt 文件",
        en: "Decode all txt files in current directory",
    },
    "decode.example.decode.recursive": {
        zh: "递归解码所有目录中的 txt 文件",
        en: "Recursively decode all txt files in all directories",
    },
    "decode.example.decode.from.gbk": {
        zh: "从 GBK 编码解码为 UTF-8",
        en: "Decode from GBK encoding to UTF-8",
    },
    ...
    "move.no.files.found": {
        zh: "未找到可处理的媒体文件。",
        en: "No media files found to process.",
    },
    "move.no.files.left.after.rules": {
        zh: "应用文件名规则后没有剩余文件。",
        en: "No files left after applying file name rules.",
    },
    ...
    "moveup.flat": {
        zh: "移动所有文件到输入目录根目录，不创建子目录",
        en: "move all files to input root dir, do not create sub dirs",
    },
    ...
    "prefix.mode": {
        zh: "文件名前缀模式 (auto/dir/prefix/media/seq/clean)",
        en: "filename prefix mode (auto/dir/prefix/media/seq/clean)",
    },
    ...
    "prefix.mprefix": { zh: "模式指定前缀", en: "mode prefix string" },
    "prefix.output": { zh: "输出目录", en: "output directory" },
    "prefix.suffix": {
        zh: "文件名后缀模式 (date/time/seq/size)",
        en: "filename suffix mode (date/time/seq/size)",
    },
    "prefix.suffix.seq": { zh: "序号后缀", en: "sequence suffix" },
    "prefix.seq.start": { zh: "序号的起始值", en: "sequence start value" },
    "prefix.seq.pad": { zh: "序号补零位数", en: "sequence padding digits" },

    // ========== run 命令 ==========
    "run.description": {
        zh: "运行独立任务（占位命令，尚未实现）",
        en: "Run standalone tasks (placeholder, not implemented yet)",
    },
    "run.not_implemented": {
        zh: "该子命令尚未实现，当前仅作占位；请使用 dcim/compress/remove/move/pick/rename/zipu 等具体命令。",
        en: "This subcommand is not implemented yet (placeholder only); please use dcim/compress/remove/move/pick/rename/zipu instead.",
    },
    ...
    "rename.nothing.to.do": {
        zh: "没有需要处理的文件，无需操作。",
        en: "No files need processing, nothing to do.",
    },
    ...
    "rename.mode": {
        zh: "重命名模式: clean|zhcn|replace|fixenc|mergedir|suffix|prefix",
        en: "rename mode: clean|zhcn|replace|fixenc|mergedir|suffix|prefix",
    },
    "rename.errors.summary": {
        zh: "处理过程中发生 {{count}} 个错误:",
        en: "{{count}} errors occurred during processing:",
    },
    "rename.counter": {
        zh: "添加序列号/计数器 (如 {n}, {nn}, {nnn})",
        en: "Add counter/sequence number (e.g., {n}, {nn}, {nnn})",
    },
    "rename.counter.start": {
        zh: "计数器起始值",
        en: "Counter start value",
    },
    "rename.case": {
        zh: "文件名大小写转换: upper|lower|title|sentence",
        en: "Filename case conversion: upper|lower|title|sentence",
    },
    "rename.truncate": {
        zh: "截断文件名到指定长度",
        en: "Truncate filename to specified length",
    },
    "rename.truncate.suffix": {
        zh: "文件名截断后的后缀",
        en: "Suffix to add after truncation",
    },
    "rename.add.prefix": {
        zh: "添加固定前缀到文件名",
        en: "Add fixed prefix to filename",
    },
    "rename.add.suffix": {
        zh: "添加固定后缀到文件名",
        en: "Add fixed suffix to filename",
    },
    "rename.change.ext": {
        zh: "批量修改文件扩展名",
        en: "Change file extension in bulk",
    },
    "rename.lower.ext": {
        zh: "扩展名转小写",
        en: "Convert extension to lowercase",
    },
    "rename.upper.ext": {
        zh: "扩展名转大写",
        en: "Convert extension to uppercase",
    },
    "rename.preview": {
        zh: "预览重命名结果，不实际执行",
        en: "Preview rename results without executing",
    },
    "rename.preview.format": {
        zh: "预览输出格式: json|csv|text",
        en: "Preview output format: json|csv|text",
    },
    "rename.preview.output": {
        zh: "预览结果输出到文件",
        en: "Output preview results to file",
    },
    "rename.template": {
        zh: "文件名模板，支持 {yyyy}, {mm}, {dd}, {name}, {original}",
        en: "Filename template, supports {yyyy}, {mm}, {dd}, {name}, {original}",
    },
    ...
    "pick.copy.no.files": {
        zh: "没有需要复制的文件，跳过复制步骤。",
        en: "No files to copy, skipping copy step.",
    },
    "pick.copy.skip.exists": {
        zh: "[跳过][{{index}}] 已存在: {{path}}",
        en: "[Skip][{{index}}] Exists: {{path}}",
    },
    "pick.copy.skip.exists.count": {
        zh: "跳过已存在的 {{count}} 个文件",
        en: "Skipped {{count}} existing files",
    },
    "pick.copy.done": {
        zh: "[已复制][{{index}}] {{name}} -> {{dest}} ({{size}})",
        en: "[Copied][{{index}}] {{name}} -> {{dest}} ({{size}})",
    },
    ...
    "option.pick.exclude.dir": {
        zh: "排除指定目录中的文件（仅根据文件名匹配）",
        en: "Exclude files in specified dir (filename match only)",
    },
    "pick.entries.excluded": {
        zh: "跳过排除目录中的 {{count}} 个文件",
        en: "Skipped {{count}} files from exclude dir",
    },
    // New pick keys
    "option.pick.fileList": {
        zh: "输入文件列表 (JSON 数组或文本文件)",
        en: "Input file list (JSON array or text file)",
    },
    "option.pick.dayLimit": {
        zh: "每日数量限制（默认 {{count}}）",
        en: "Maximum files picked per day (default {{count}})",
    },
    "pick.using.file.list": {
        zh: "使用文件列表: {{path}}",
        en: "Using file list: {{path}}",
    },
    "pick.file.list.not.found": {
        zh: "文件列表未找到: {{path}}",
        en: "File list not found: {{path}}",
    },
    "pick.json.missing.field": {
        zh: "无效的 JSON 项: 缺少必要的 'path' 或 'size' 字段: {{item}}",
        en: "Invalid JSON item: missing required 'path' or 'size' field: {{item}}",
    },
    "pick.json.must.be.array": {
        zh: "JSON 文件必须包含数组",
        en: "JSON file must contain an array",
    },
    "pick.json.parse.failed": {
        zh: "解析 JSON 文件列表失败: {{error}}",
        en: "Failed to parse JSON file list: {{error}}",
    },
    "pick.file.list.format.unknown": {
        zh: "不支持的文件列表格式: {{ext}}。请使用 .json 或 .txt",
        en: "Unsupported file list format: {{ext}}. Please use .json or .txt",
    },
    "pick.loaded.entries": {
        zh: "从文件列表加载了 {{count}} 个条目",
        en: "Loaded {{count}} entries from file list",
    },
    "pick.burst.removed": {
        zh: "Burst 模式已移除 {{count}} 张重复照片",
        en: "Burst mode removed {{count}} duplicate photos",
    },
    "pick.hash.removed": {
        zh: "感知哈希去重已移除 {{count}} 张相似照片",
        en: "Image hash dedup removed {{count}} similar photos",
    },
    "option.pick.burstMode": {
        zh: "Burst 模式: off/auto/aggressive",
        en: "Burst mode: off/auto/aggressive",
    },
    "option.pick.hashDedup": {
        zh: "启用感知哈希去重",
        en: "Enable perceptual hash deduplication",
    },
    "option.pick.hashThreshold": {
        zh: "感知哈希相似度阈值 (0-64, 越小越严格)",
        en: "Hash similarity threshold (0-64, smaller is stricter)",
    },
    "option.pick.cache": {
        zh: "启用哈希缓存（加速多次运行）",
        en: "Enable hash cache (speed up multiple runs)",
    },
    "option.pick.cacheFile": {
        zh: "指定缓存文件路径（默认在输出目录）",
        en: "Specify cache file path (default in output directory)",
    },
    "option.pick.noCache": {
        zh: "禁用缓存，重新计算所有特征",
        en: "Disable cache, recalculate all features",
    },
}

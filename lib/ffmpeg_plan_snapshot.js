...
export function createPublicPlanSnapshot(plan = {}) {
    ...
    return {
        ...
        tasks: tasks.map((task, index) => ({
            ...
            videoCodec: task.videoCodec || task.srcCodec || undefined,
            width: Number.isFinite(task.width) ? task.width : undefined,
            height: Number.isFinite(task.height) ? task.height : undefined,
            fps: Number.isFinite(task.fps) ? task.fps : undefined,
        })),
    }
}

...
export function createObjectWithKeyField(obj, fieldName = "key") {
    // 用 Object.fromEntries 而非 `out[key] = value` 累积：
    // 后者在 key 为 "__proto__" 时走的是原型 setter，会写坏结果对象的原型
    // （这两处的 key 现在来自用户可编辑的 presets.yaml）。
    return Object.fromEntries(
        Object.keys(obj)
            .filter((key) => hasOwn(obj, key))
            .map((key) => {
                const value = obj[key]
                return [key, isExtendableObject(value) ? { ...value, [fieldName]: key } : value]
            }),
    )
}
...
export function removeFieldsIn(obj, removeKeys = []) {
    for (const key in obj) {
        if (hasOwn(obj, key) && removeKeys.includes(key)) {
            delete obj[key]
        }
    }
}

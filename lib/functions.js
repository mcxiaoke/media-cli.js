...
async function makeThumbHEIF(t) {
    log.info("makeThumb[HEIF]", "processing", t)
    if (helper.isHEVCImage(t.src)) {
        const exePath = await which("nconvert", { nothrow: true })
        if (!exePath) {
            log.warn("makeThumb[HEIF]", "nconvert executable not in path")
            return
        }
        ...
        try {
            const { stderr } = await $({ encoding: 'binary' })`${cmd} -overwrite -opthuff -no_auto_ext -out jpeg -o ${dstName} -q ${t.quality} -resize longest ${t.width} ${fileSrc}`;
            const sr = fixEncoding(stderr)
            log.info("makeThumb[HEIF]", "stderr", sr)
            // strange fix for encoding str compare
            if (sr.endsWith(fixedOkStr)) {
                log.show("makeThumb[HEIF]", `${helper.pathShort(fileSrc)} => ${dstName}`)
                return {
                    width: t.width,
                    height: t.height,
                    format: 'jpeg'
            ...
            log.error("makeThumb[HEIF]", error)
        }

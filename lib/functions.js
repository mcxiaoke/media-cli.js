...
export async function compressImage(t) {
    ...
    try {
        ...
        if (fst.size < 100 * 1024) {
            ...
            log.showYellow(logTag, `Delete: ${t.index}/${t.total}`, `<${helper.pathShort(t.dst)}>`, `${helper.humanSize(fst.size)}`, chalk.yellow(`file corrupted`));
            log.fileLog(`Delete: ${t.index}/${t.total} <${helper.pathShort(t.dst)}> ${helper.humanSize(fst.size)} file corrupted`, logTag);
        }
        t.dstExists = await fs.pathExists(t.dst);
        t.dstExists && log.fileLog(`Done: <${t.src}> => ${path.basename(t.dst)} ${helper.humanSize(fst.size)}`, logTag);
        // 返回处理后的图像信息对象  
        t.done = true;
        return t;
    } catch (error) {

...
import { HANZI_COMMON_3500, HANZI_RARELY_USED, strHasASCII, strHasHFKanaHira, strHasHiraKana, strOnlyASCII, strOnlyChinese, strOnlyHangul, strOnlyJapanese, strOnlyJapaneseHan } from './unicode.js'
...
export const MESSY_CJK_CHARS = HANZI_RARELY_USED

export const REGEX_MESSY_CJK = new RegExp(`[${MESSY_CJK_CHARS}]`, 'u')

export const REGEX_MESSY_CJK_EXT = /[\u8823-\u883f]/u //生僻字: 虫字旁 金字旁
...
export function showBadCJKChars(str) {
    ...
    Array.from(str).forEach(c => {
        if (REGEX_MESSY_CJK.test(c)) {
            chars.push(c)
        }
        if (REGEX_MESSY_CJK_EXT.test(c)) {
            chars.push(c)
        }
    })

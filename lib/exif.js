...
async function listFiles(root) {
  ...
  const filterMedia = (item) => {
    return h.isMediaFile(item.path) && item.stats.size > 100 * 1024;
  };
  ...
  d.I(`listFiles ${files.length} files in ${Date.now() - startMs}ms`);
  return files;
}
...
async function parseFiles(files) {
  ...
  const exifFiles = await Promise.all(
    files.map(async (f) => {
      d.D(`parseFiles start:${f.path}`);
      const { date, exif } = await getExifDate(f.path);
      d.D(`parseFiles end:${f.path} ${date}`);
      return (
        date && {
          path: f.path,
          root: f.root,
  ...
  await et.end();
  d.I(`parseFiles ${exifFiles.length} in ${Date.now() - startMs}ms`);
...
function createNameByDate(file) {
  ...
  d.D(`createNameByDate ${h.sps(file.path)} ${file.outName}}`);
  return file;
}
...
function buildNames(files) {
  ...
  d.I(`buildNames time: ${Date.now() - startMs}ms`);
  return newFiles;
}
...
function checkFiles(files) {
  ...
  files = files.filter((f) => {
    ...
    if (
      ...
      d.I(chalk.gray(`Skip [Name]: ${h.sps(f.path)} <${f.outName}>`));
      return false;
    } else {
      return true;
    }
  ...
  files = files.map((f) => {
    ...
    d.L(chalk.green(`Prepared:`) + ` ${h.sps(f.path)} ==> ${f.outName}`);
    return f;
  });
  return files;
}

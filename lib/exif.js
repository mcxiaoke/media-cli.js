...
async function readAllTags(files) {
  ...
  const readExifOne = async (f, i) => {
    ...
    log.info(
      ...
      helper.pathShort(filename), tags?.Model || tags?.Software || tags.Make || '(Model)', extractExifDate(tags) || '(Date)')
    f.tags = tags
    // } catch (error) {
...
function fixP950Tag(tags) {
  ...
  if (tags.Model &&
    tags.Model.toLowerCase().includes("p950")) {
...
function selectDateTag(tags) {
  ...
  for (const k of keys) {
    if (tags[k] instanceof exiftool.ExifDateTime) {
      ...
      console.log(tags.SourceFile, dt.hour, k, dt)
      return [k, dt]
    }
  }
}
    ...
    files.map(async (f) => {
      ...
      return (
        date && {
          ...
          date: date[1].toDate(),
          rawDate: date,
          model: f.tags && (f.tags.Model || f.tags.Make || f.tags.MajorBrand || f.tags.HandlerVendorID),
          // rawExif: exif,
        }
      )
    })
...
function createNameByDate(file) {
  ...
  let dstName = ms > 0 ? `${prefix}${dateStr}_${ms}${suffix}${ext}`
    : `${prefix}${dateStr}${suffix}${ext}`

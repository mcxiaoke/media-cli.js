...
async function readAllTags(files) {
  ...
  log.isDebugMode() || bar1.start(t, 0);
  const readExifOne = async (f, i) => {
    const filename = f.path;
    log.isDebugMode() || bar1.increment();
    try {
      ...
      log.info(
        ...
        helper.pathShort(filename, 30), tags?.Model || tags?.Software || tags.Make, selectDateTag(tags)[1]?.rawValue);
      f.tags = tags;
    } catch (error) {
...
function hackAndFix(tags) {
  return fixAppleTag(tags) || fixScreenShot(tags);
}
...
function checkFiles(files) {
  ...
  files = files.filter((f) => {
    if (helper.isVideoFile(f.path) && f.size < 500 * 1024) {
      log.info(
        `Check [Size]:`,
        `${helper.pathShort(f.path)} <${helper.fileSizeSI(f.size)}>`
      );

...
export async function getSimpleInfo(filePath) {
    ...
    if (info?.audio) {
        const a = info?.audio
        arr.push(`a:${a.codec_name},${roundNum(a.bit_rate / 1024)}K,${Math.floor(a.duration)}s`)
    }
    if (info?.video) {
        const v = info?.video
        arr.push(`v:${v.codec_name},${roundNum(v.bit_rate / 1024)}K,${Math.floor(v.duration)}s,${v.r_frame_rate},${v.width}x${v.height}`)
    }

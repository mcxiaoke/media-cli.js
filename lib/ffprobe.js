...
import { humanDuration, humanSize } from './helper.js'
...
export async function getMediaInfo(filePath, options = { video: false, audio: false, fullData: false }) {
    ...
    const propsSelected = 'stream=codec_name,codec_long_name,profile,codec_type,codec_tag_string,width,height,display_aspect_ratio,pix_fmt,duration,bit_rate,sample_rate,sample_fmt,time_base,r_frame_rate,avg_frame_rate,channels,bits_per_sample:format=format_name,format_long_name,duration,size,bit_rate'
    ...
    try {
        ...
        log.error(`getMediaInfo:`, filePath, error.message)
    }
}

export async function getSimpleInfo(filePath) {
    const info = await getMediaInfo(filePath)
    const text = []
    if (info?.video) {
        const v = info?.video
        text.push(`${v.codec_type},${v.codec_name},${humanSize(v.bit_rate)},${Math.floor(v.duration)}s,${v.r_frame_rate},${v.width}x${v.height}`)
    }
    if (info?.audio) {
        const a = info?.audio
        text.push(`${a.codec_type},${a.codec_name},${humanSize(a.bit_rate)},${Math.floor(a.duration)}s`)
    }
    return text.join(' ; ')
}

function parseFrameRate(fpsStr) {
    if (typeof fpsStr !== 'string') {
        return 0
    }
    const regex = /(\d+)\/(\d+)/
    const match = fpsStr.match(regex)

...
function parseValue(value) {
    // Check if value is a floating point number
    if (!isNaN(parseFloat(value)) && isFinite(value) && !value.includes('"') && !value.includes("'")) {
        return parseFloat(value)
    }
    // Check if value is 'true' or 'false'
    else if (value.toLowerCase() === 'true') {
    ...
    // Check if value is a comparison expression
        ...
        const number = parseFloat(value.substr(operator.length))
...
// sample input
const sampleInput = "a=[1.5,2,3,abd,hello];b=[fdsf,rer,wr,w43,rrerr,2024.4];ab=12.8;aq=3.1425;ac=aac;ap=aac_he;vb=1536;vq=23;vc=hevc_nvenc;vs=1280*720;ddf;dfdfdfaa=;test mode='3434324':yes=false;width=>1200;height=<600;nothing"
console.log(parseString(sampleInput))

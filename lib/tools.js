 ...
 * @returns {Promise<string>} 16进制 hash
 */
export async function hashFileXXH64Partial(path, maxBytes = 20 * 1024 * 1024, seed = 0) {
    const { create64 } = await xxhash()
    const hasher = create64()
    const stream = fs.createReadStream(path, { highWaterMark: 1024 * 1024 }) // 1MB块
    let readBytes = 0
 ...
 * @param {string} path1
 * @param {string} path2
 * @param {number} maxBytes - 默认 20MB
 * @returns {Promise<boolean>}
 */
export async function isSameFileXXH64Partial(path1, path2, maxBytes = 20 * 1024 * 1024) {
    try {
...
export async function isSameFileCached(path1, path2, maxBytes = 20 * 1024 * 1024) {
    const [stat1, stat2] = await Promise.all([fsp.stat(path1), fsp.stat(path2)])
    if (stat1.size !== stat2.size) return false

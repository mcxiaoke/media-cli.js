 ...
 * 计算字符串的MD5短哈希值
 * @param {string} str - 要计算哈希的字符串
 * @param {number} limit - 截取长度，默认8位
 * @returns {string} 指定长度的十六进制哈希值
 */
export const md5Short = (str, limit = 8) => md5Hash(str).slice(0, limit)
 ...
 */
export async function hashFileXXH64(path) {
 ...
 * @returns {Promise<string>} 16进制 hash
 */
export async function hashFileXXH64Partial(path, maxBytes = XXHASH_PART_SIZE) {
    const { create64 } = await xxhash()
    const hasher = create64()
    const stream = fs.createReadStream(path, { highWaterMark: 1024 * 1024 }) // 1MB块
    let readBytes = 0
...
export async function isSameFileXXH64Partial(path1, path2, maxBytes = XXHASH_PART_SIZE) {
    try {
        ...
        log.error("判断文件相同出错:", err)
        return false
    }
}

// 哈希缓存：用于 isSameFileCached 避免重复计算。
// 需限制容量，否则长时间运行（批量处理数万文件）会持续增长。
const hashCache = new Map()
const HASH_CACHE_MAX = 50000

/**
 * 读取哈希缓存
 *
 * 必须校验 size 与 mtimeMs：仅以路径为键的话，文件被改写后仍会返回旧哈希，
 * 导致两个已经不同的文件被判为相同（DUP），进而被跳过移动、静默丢失处理。
 *
 * @param {string} key - 文件路径
 * @param {Object} stat - fsp.stat 结果
 * @returns {string|null} 命中且有效时返回哈希，否则 null
 */
function getHashCache(key, stat) {
    const hit = hashCache.get(key)
    if (!hit) {
        return null
    }
    if (hit.size !== stat.size || hit.mtimeMs !== stat.mtimeMs) {
        // 文件已变化，旧哈希不可再用
...
/**
 * 写入哈希缓存，超过容量上限时按插入顺序淘汰最旧条目（FIFO）
 *
 * @param {string} key - 文件路径
 * @param {string} value - 哈希值
 * @param {Object} stat - fsp.stat 结果，用于后续失效判定
 */
function setHashCache(key, value, stat) {
    if (hashCache.size >= HASH_CACHE_MAX && !hashCache.has(key)) {
        // Map 保持插入顺序，取第一个键即为最旧
...
export async function isSameFileCached(path1, path2, maxBytes = XXHASH_PART_SIZE) {
    ...
    const hash1 = getHashCache(path1, stat1) || (await hashFileXXH64Partial(path1, maxBytes))
    const hash2 = getHashCache(path2, stat2) || (await hashFileXXH64Partial(path2, maxBytes))

    setHashCache(path1, hash1, stat1)
    setHashCache(path2, hash2, stat2)

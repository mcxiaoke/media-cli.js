...
const PRESET_FIELD_DEFS = {
    ...
    videoArgs: {
        ...
        construct: true,
        comment:
            "视频参数串（硬件无关部分）。S-4：禁止再出现 -c:v 硬编码编码器，" +
            "编码器由 lib/hwaccel.js 按 tier + videoCodecFamily 决定",
    },
    audioArgs: { type: "string", construct: true, comment: "音频参数串" },
    inputArgs: { type: "string", construct: true, comment: "输入参数串" },
    streamArgs: { type: "string", construct: true, comment: "流映射/元数据参数串" },
    extraArgs: { type: "string", construct: true, comment: "附加参数串" },
    outputArgs: {
        type: "string",
        construct: true,
    ...
    complexFilter: {
        ...
        construct: true,
        comment:
            "复杂滤镜串（与 filters 互斥），同样支持 {scaleFilter} 占位符与 {speed} 等模板变量",
    },
    pre_filters: {
        type: "string",
        construct: true,
    ...
    videoBitrate: { type: "number", construct: true, comment: "视频码率（bps）" },
    videoQuality: {
        type: "number",
        construct: true,
        comment: "视频质量值（CRF/CQ 语义，由 tier 层映射为 -crf/-cq/-global_quality 等）",
    },
    audioBitrate: { type: "number", construct: true, comment: "音频码率（bps）" },

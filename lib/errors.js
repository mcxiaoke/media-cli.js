...
/**
 * 错误严重程度级别
 * 用于确定错误的处理方式和是否需要终止程序
 */
export const ErrorSeverity = {
    INFO: "INFO",   // 信息性错误，不影响程序运行
    WARN: "WARN",   // 警告，可能的问题但可以继续
    ERROR: "ERROR", // 一般错误，需要处理但可能恢复
    FATAL: "FATAL", // 致命错误，必须终止程序
}
...
/**
 * 自定义错误类 - MediaCli专用错误类型
 * 扩展标准Error类，添加类型、严重程度等额外信息
 */
...
export class ErrorHandler {
    ...
    setupDefaultHandlers() {
        // 文件不存在错误
        this.registerHandler(ErrorTypes.FILE_NOT_FOUND, (error) => {
        ...
        // 权限错误
        this.registerHandler(ErrorTypes.FILE_ACCESS_DENIED, (error) => {
        ...
        // 格式不支持错误
        this.registerHandler(ErrorTypes.UNSUPPORTED_FORMAT, (error) => {
        ...
        // 处理失败错误
        this.registerHandler(ErrorTypes.PROCESSING_FAILED, (error) => {
        ...
        // 参数错误
        this.registerHandler(ErrorTypes.INVALID_ARGUMENT, (error) => {
        ...
        // 外部工具错误
        this.registerHandler(ErrorTypes.FFMPEG_ERROR, (error) => {
    ...
    async handle(error, context = {}) {
        ...
        if (mediaCliError.severity === ErrorSeverity.FATAL) {
            ...
            // 不再直接 process.exit()：库层强制终止进程会让调用方无法清理资源
            // 也无法在测试中观察。改为抛出带退出码的错误，由入口统一决定退出。
            const fatalError = createError(
                mediaCliError.type,
                mediaCliError.message,
                mediaCliError.originalError,
                ErrorSeverity.FATAL,
            )
            fatalError.exitCode = exitCode
            throw fatalError
        }

...
async function doDetectHardwareCapabilities({ ffmpegPath, deviceProbe }) {
    ...
    const [verRes, encRes, hwRes, fltRes] = await Promise.all([
...
/**
 * 打印软硬件环境信息（单条 INFO，含 ffmpeg 构建/编码器/硬件加速栈）
...
export function candidateTiers(caps, { decodeMode = "auto", hwaccel } = {}) {
    ...
    // auto（默认链）：厂商专属层（若可用且滤镜齐）→ cpu
    // ⚠️ T5：d3d 层从自动候选链移除 —— d3d 是「任意厂商解码 + NVENC 编码」的混合层，
    //    auto 语义下多数机器它都不是最优（AMD/Intel 机器 nvenc 编码器不存在），
    //    只有显式 --hwaccel d3d/d3d11va/d3d12va（或 decodeMode=gpu）才使用。
    //    PROBE 白名单分支天然不受影响（显式指定仍可覆盖）。
    const list = []

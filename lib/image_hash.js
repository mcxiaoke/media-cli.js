...
export async function computeAverageHash(filePath, options = {}) {
    ...
    try {
        ...
        // 无 alpha 通道时为 no-op；消除 stats() 预检导致的重复解码（P2-5）
        image.removeAlpha()
...
export async function calculateImageQualityScore(filePath, config = QUALITY_CONFIG) {
    try {
        ...
        // 复用已解码的灰度数据做内存版拉普拉斯卷积，避免再次解码文件（P2-5）
        const sharpnessVariance = calculateLaplacianVarianceData(data, info.width, info.height)
...
export async function calculateLaplacianVariance(filePath) {
    try {
        ...
        return calculateLaplacianVarianceData(data, info.width, info.height)
    } catch (e) {
        return 0
    }
}
...
/**
 * 计算灰度像素数据的拉普拉斯方差（内存版，不涉及文件 IO）
 *
 * 与 calculateLaplacianVariance 的 3x3 拉普拉斯核一致（[0,1,0;1,-4,1;0,1,0]），
 * 供已持有像素数据的调用方直接复用，避免重复文件解码（P2-5）。
 *
 * @param {Uint8Array} data - 灰度图像数据（raw 输出）
 * @param {number} width - 图像宽度
 * @param {number} height - 图像高度
 * @returns {number} 拉普拉斯方差
 */
function calculateLaplacianVarianceData(data, width, height) {
    if (!data || data.length === 0 || width < 3 || height < 3) return 0
        ...
        for (let x = half; x < width - half; x++) {
            ...
            for (let ky = 0; ky <= 2; ky++) {
                for (let kx = 0; kx <= 2; kx++) {
                    const idx = (y + ky - half) * width + (x + kx - half)

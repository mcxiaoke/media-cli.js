 ...
 * @param {AbortSignal} [options.signal] - 取消探测的信号
 * @param {string} [options.mediainfoPath] - 显式指定的 mediainfo 可执行文件（缺省用 PATH 中的 mediainfo）
 * @returns {Promise<Object>} MediaInfo输出的JSON数据
 */
async function mediainfoCall(filePath, { signal = null, mediainfoPath = null } = {}) {
    const cmdArgs = ["--Output=JSON", filePath]
    // 使用 execa 执行 mediainfo 命令
...
async function parseMediaInfo(filePath, options = {}) {
    return fromMediaInfoJson(
        await mediainfoCall(filePath, {
            signal: options.signal,
            mediainfoPath: options.mediainfoPath,
        }),
    )
}
 ...
 * @param {string} [options.mediainfoPath] - 显式指定的 mediainfo 可执行文件路径
...
export async function getMediaInfo(filePath, options = {}) {
    ...
    // 显式传入 mediainfoPath 时视为可用（不再依赖模块加载时的 PATH 探测结果）
    const hasMediaInfo = Boolean(options.mediainfoPath) || HAS_MEDIAINFO_EXE
    if (!hasProbe && !hasMediaInfo) {

...
export async function getSimpleInfo(filePath, options = {}) {
    ...
    arr.push(`format=${info?.format},ts=${info?.duration}s,bit=${roundNum((info?.bitrate || 0) / 1000)}K`)
    if (info?.audio) {
        const a = info?.audio
        arr.push(`ac=${a.format},ts=${a.duration}s,bit=${roundNum((a.bitrate || 0) / 1000)}K`)
    }
    if (info?.video) {
        const v = info?.video
        arr.push(`vc=${v.format},ts=${v.duration}s,bit=${roundNum((v.bitrate || 0) / 1000)}K,fps=${v.framerate},dm=${v.width}x${v.height}`)
    }

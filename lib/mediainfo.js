 ...
 */
async function ffprobeCall(filePath, probePath = ffprobeBin) {
    ...
    const { stdout } = await execa(probePath, cmdArgs, { encoding: "latin1" })
...
async function mediainfoCall(filePath) {
    ...
    const { stdout } = await execa(mediaInfoBin, cmdArgs, { encoding: "latin1" })
    // 解决windows下文件名乱码问题
    // 方法1 乱码部分文本直接正则清除掉
...
async function parseFFProbe(filePath, options = {}) {
    return fromFFprobeJson(await ffprobeCall(filePath, options.ffprobePath || ffprobeBin))
}
...
export async function getMediaInfo(filePath, options = { useMediaInfo: false }) {
    ...
    let [err1, data1] = await trySmartAsync(options.useMediaInfo ? parseMediaInfo : parseFF)(
        filePath,
    )

    if (err1) {
        // 首选解析器失败，尝试备选解析器
        let [err2, data2] = await trySmartAsync(options.useMediaInfo ? parseFF : parseMediaInfo)(
            filePath,
        )

...
function fromFFprobe(data) {
    const obj = {
        ...
        duration: data['duration']
            || data['tags']?.['DURATION']
            || data['tags']?.['DURATION-eng'], // 时长
        bitrate: data['bit_rate']
            || data['tags']?.['BPS']
            || data['tags']?.['BPS-eng'], // 比特率
...
function fromMediaInfo(data) {
    ...
    const obj = {
        ...
        // 帧率
        framerate: data['FrameRate'] || (data['FrameRate_Num'] + '/' + data['FrameRate_Den']),
...
export function fromFFprobeJson(data) {
    ...
    const sd = data.streams?.filter(obj => obj.codec_type === "subtitle")
    const obj = {
        provider: 'ffprobe',
        ...
        subtitles: sd && sd.map(fromFFprobe),
    }
    // console.log('MediaInfo.fromFFprobeJson obj', obj)
    const info = new MediaInfo(convertNumber(obj))
    // console.log('MediaInfo.fromFFprobeJson info', info)
...
const FPS_KEY_LIST = ['r_frame_rate', 'avg_frame_rate']
...
function convertNumber(obj) {
    ...
    for (const [key, value] of Object.entries(obj)) {
        ...
        // 检查属性的值是否为字符串类型且可以转换为数字
            ...
            if (key.includes('frame_rate') || key.includes('framerate')) {

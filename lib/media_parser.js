 ...
 *    "未知"由上层决定方向，见 src/transcode/hwaccel.js `needsDepthAlign` 的保守处理
 *    （位深未知按"需要对齐"处理）。
 *
 ...
 * ⚠️ 比较"是否需要改帧率"时另需 ±2% 容差，见 src/transcode/ffmpeg_plan.js。
 */
...
function fromFFprobe(data) {
    ...
    // src/transcode/ffmpeg_plan.js 的 dstScaleNeeded、src/transcode/ffmpeg_build.js 的
    // calcLongEdge、src/transcode/ffmpeg_run.js 的 selectTier srcW/srcH —— 三处都该用显示尺寸）。

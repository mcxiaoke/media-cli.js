...
const H264_BASE = new FFmpegPreset('h264-base', {
    ...
    videoArgs: VCODEC_NVENC_H264 + ' -cq {videoQuality} -bufsize {videoBitrate}k -maxrate {videoBitrate}k',
    // 音频参数说明
...
const HEVC_QSV_BASE = new FFmpegPreset('hevc-qsv-base', {
    ...
    // video_quality block '-cq {quality} -bufsize {bitrate} -maxrate {bitrate}'
    videoArgs: VCODEC_QSV_HEVC + ' -global_quality {videoQuality} -bufsize {videoBitrate}k -maxrate {videoBitrate}k',
    // 音频参数说明
    // audio_codec block '-c:a libfdk_aac'
    // audio_quality block '-b:a {bitrate}'
    audioArgs: '-c:a libfdk_aac -b:a {audioBitrate}k',
...
const HEVC_BASE = new FFmpegPreset('hevc-base', {
    ...
    // video_codec block '-c:v hevc_nvenc -profile:v main -tune:v hq'
    // video_quality block '-cq {quality} -bufsize {bitrate} -maxrate {bitrate}'
    videoArgs: VCODEC_NVENC_HEVC + ' -cq {videoQuality} -bufsize {videoBitrate}k -maxrate {videoBitrate}k',
    // 音频参数说明
    // audio_codec block '-c:a libfdk_aac'
    // audio_quality block '-b:a {bitrate}'
    audioArgs: '-c:a libfdk_aac -b:a {audioBitrate}k',

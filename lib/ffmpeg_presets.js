...
import { loadPresetsFromYaml, mergePresets } from "./preset_loader.js"
...
// H264 AVC
const VCODEC_NVENC_H264 = "-c:v h264_nvenc -rc vbr -tune hq"
//  HEVC -profile:v main -tune:v hq
const VCODEC_NVENC_HEVC =
    "-c:v hevc_nvenc -rc vbr -tune hq -rc-lookahead 24 -spatial-aq 1 -temporal-aq 1"
//  HEVC QSV
...
const H264_BASE = new FFmpegPreset("h264-base", {
    ...
    videoArgs:
        VCODEC_NVENC_H264 + " -cq {videoQuality} -bufsize {videoBitrateK} -maxrate {videoBitrateK}",
    // 音频参数说明
    ...
    filters:
        "scale_cuda=w={dimension}:h={dimension}:force_original_aspect_ratio=decrease:interp_algo=lanczos,format=cuda",
    complexFilter: "",
})
// AVC H264
...
const HEVC_QSV_BASE = new FFmpegPreset("hevc-qsv-base", {
    ...
    videoArgs:
        VCODEC_QSV_HEVC +
        " -global_quality {videoQuality} -bufsize {videoBitrateK} -maxrate {videoBitrateK}",
    // 音频参数说明
    // audio_codec block '-c:a libfdk_aac'
    // audio_quality block '-b:a {bitrateK}'
    audioArgs: "-c:a libfdk_aac -b:a {audioBitrateK}",
    ...
    // -vf 'scale=if(gte(iw\,ih)\,min(1280\,iw)\,-2):if(lt(iw\,ih)\,min(1280\,ih)\,-2)'
    filters:
        "scale_qsv='if(gte(iw,ih),min({dimension},iw),-2)':'if(lt(iw,ih),min({dimension},ih),-2)'",
    complexFilter: "",
})
...
const HEVC_BASE = new FFmpegPreset("hevc-base", {
    ...
    // video_quality block '-cq {quality} -bufsize {bitrateK} -maxrate {bitrateK}'
    videoArgs:
        VCODEC_NVENC_HEVC + " -cq {videoQuality} -bufsize {videoBitrateK} -maxrate {videoBitrateK}",
    // 音频参数说明
    // audio_codec block '-c:a libfdk_aac'
    // audio_quality block '-b:a {bitrateK}'
    audioArgs: "-c:a libfdk_aac -b:a {audioBitrateK}",

...
//  -profile:v main -tune:v hq
const VCODEC_HEVC = '-c:v hevc_nvenc'
...
const PRESET_HEVC_SPEED = FFmpegPreset.fromPreset(HEVC_BASE).update({
    ...
    complexFilter: "[0:v]setpts=PTS/{speed},scale_cuda='if(gte(iw,ih),min({dimension},iw),-2)':'if(lt(iw,ih),min({dimension},ih),-2)':interp_algo=lanczos[v];[0:a]atempo={speed}[a]"
})

...
class FFmpegPreset {
    constructor(
        ...
        this.userArgs = {
            ...
            audioCopy: false,
            videoCopy: false,
            // 定稿新增：追加式自定义参数通道（build 阶段追加到对应段末尾，后写覆盖）
            videoExtra: "", // 来自 --video-args
            audioExtra: "", // 来自 --audio-args
...
function applyFfargs(argv, ffargs) {
    ...
    for (const [key, value] of Object.entries(ffargs)) {
        ...
        } else if (normalizedKey === "filters" || normalizedKey === "filterComplex") {
            if (!hasArgvValue && typeof value === "string" && value.length > 0) {
                result[normalizedKey] = value
            }
...
function createFromArgv(argv) {
    ...
    // --video-args / --audio-args 改为「追加」语义（定稿）：
    // 不再整体替换 preset.videoArgs / preset.audioArgs——那会连带丢弃预设自带的基线参数，
    // 且 --audio-args 覆盖 preset.audioArgs 会绕过 buildAudioArgs 的 copy/降级智能判断。
    // 用户串存入 userArgs.videoExtra / audioExtra，由 ffmpeg_build 追加到各段末尾
    // （靠 ffmpeg「后写覆盖」取得最高优先级）。
    if (typeof argv.videoArgs === "string" && argv.videoArgs.trim().length > 0) {
        preset.userArgs.videoExtra = argv.videoArgs.trim()
    }
    if (typeof argv.audioArgs === "string" && argv.audioArgs.trim().length > 0) {
        preset.userArgs.audioExtra = argv.audioArgs.trim()
    }
    // --filters 改为「追加到后滤镜段」：不再整体覆盖 preset.filters，
    // 从而保留 "{scaleFilter}" 占位符（否则缩放失效，v1 §2.3 的坑）。
    if (typeof argv.filters === "string" && argv.filters.trim().length > 0) {
        const f = argv.filters.trim()
        preset.post_filters = preset.post_filters ? `${preset.post_filters},${f}` : f
    }

...
function buildInputArgs(entry, tempPreset, hwPlan) {
    const inputArgs = []
    // 全局选项（最前）
    const globalOpts = tempPreset.userArgs?.argOptions?.global
    if (globalOpts?.length > 0) {
        inputArgs.push(...globalOpts)
    }
    ...
    // 用户自定义 @input 参数（seek、trim、格式强制等）
    const inputOpts = tempPreset.userArgs?.argOptions?.input
    if (inputOpts?.length > 0) {
        inputArgs.push(...inputOpts)
    }
    inputArgs.push("-i")

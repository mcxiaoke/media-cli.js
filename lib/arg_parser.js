...
/**
 * 按分隔符切分键值对，但只在"分隔符后确实跟着新的 key=value"时才切分。
 *
 * 背景：此前直接 `inputString.split(/;|:|#/)`，导致值中含冒号的常见写法
 * （如 `size=16:9`、时间 `12:30`、URL）被误切并抛 INVALID_FORMAT。
 * 现在只有当下一个片段仍含 `=` 时才视为新键值对的开头，
 * 因此 `a=1:b=2` 仍正确切分，而 `size=16:9` 会保持完整。
 *
 * @param {string} inputString - 原始参数串
 * @returns {string[]} 切分后的键值对片段
 */
function splitKeyValuePairs(inputString) {
    const separators = [";", ":", "#"]
    const parts = []
    let current = ""
    for (let i = 0; i < inputString.length; i++) {
        ...
        if (separators.includes(ch)) {
            // 查看剩余内容是否像一个新的 key=value
            const rest = inputString.slice(i + 1)
            const nextSepIdx = rest.search(/[;:#]/)
            const segment = nextSepIdx === -1 ? rest : rest.slice(0, nextSepIdx)
            if (segment.includes("=")) {
                parts.push(current)
                current = ""
                continue
...
export function parseArgs(inputString, config = {}) {
    ...
    const keyValuePairs = splitKeyValuePairs(inputString)
    ...
    for (const reqKey of required) {
        // 此前只检查 undefined，导致 `q=`（解析为 null）能绕过必填校验。
        // 现在把 null / 空字符串一并视为"未提供"。
        const v = parsedObject[reqKey]
        if (v === undefined || v === null || v === "") {
            throw new ArgParseError(`Missing required parameter: ${reqKey}`, "MISSING_REQUIRED")
        }
    }
...
function validateAndConvertType(rawKey, normalizedKey, value, expectedType) {
    switch (expectedType) {
        case "number":
            ...
            throw new ArgParseError(`"${rawKey}" must be a number, got: "${value}"`, "TYPE_ERROR")
        ...
        case "boolean":
            ...
            throw new ArgParseError(`"${rawKey}" must be a boolean, got: "${value}"`, "TYPE_ERROR")

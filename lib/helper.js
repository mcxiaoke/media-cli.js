 ...
 * 保留原有目录结构，防止误删重要文件
 * 删除的文件会按日期分类存储在磁盘根目录的Deleted_By_Mediac文件夹中
 *
 * @param {string} filepath - 要删除的文件路径
 * @returns {Promise<string|undefined>} 转移后的目标路径；失败时返回 undefined
 */
export async function safeRemove(filepath) {
    try {
        ...
        // 不再静默吞错：调用方需要知道删除失败，避免"报告成功实际未删"
        console.error(`[safeRemove] failed: ${filepath} - ${error?.message || error}`)
        return undefined
    }
}

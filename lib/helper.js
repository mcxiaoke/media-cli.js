...
export const VIDEO_FORMATS = [".mp4", ".mov", ".wmv", ".avi", ".mkv", ".m4v", ".ts", ".flv", ".webm", ".rmvb", ".rm", ".vob"]

export const AUDIO_FORMATS = [".aac", ".m4a", ".mp3", ".wma", ".flac", ".wav"]
...
export const MEDIA_FORMATS = [...RAW_FORMATS, ...IMAGE_FORMATS, ...VIDEO_FORMATS]
...
export function humanTime(startMs, compat = true) {
  const arr = []
  ...
  if (seconds > 0) {
    arr.push(seconds + (compat ? "S" : "Secs"))
  }
  if (minutes > 0) {
    arr.push(minutes + (compat ? "M" : "Mins"))
  }
  if (hours > 0) {
    arr.push(hours + (compat ? "H" : "Hours"))
  }
  if (days > 0) {
    arr.push(days + (compat ? "D" : "Days"))
  }
  return arr.join(compat ? "" : " ")
}

// https://stackoverflow.com/questions/19700283/
export function formatMillis(startMs, padStart = false) {
  let ms = Date.now() - startMs
  function pad(num) {

...
// Legacy catch-all：仅允许删除或修复，不再承接新职责；新工具函数应新建独立模块。
...
/**
 * MP4 系容器不支持的音频编码白名单（黑名单）
 *
 * 这些编码是各家私有/老式编码，MP4/ISOBMFF 容器没有对应的 codec tag，
 * 使用 `-c:a copy` 封装时会直接报错：
 *   Could not find tag for codec xxx in stream #1,
 *   codec not currently supported in container
 *
 * 注意：这与「解码器是否可用」无关，ffmpeg 可以正常解码这些编码，
 * 只是无法把原始码流原样塞进 MP4 容器，必须重编码。
 */
const MP4_INCOMPATIBLE_AUDIO_CODECS = [
    // RealMedia / RealAudio 系
    // 注意：这些条目需同时覆盖两条检测器路径的取值——
    // ffprobe 路径 srcAudioCodec 为 codec_name（cook/sipr/ralf…），
    // mediainfo 路径 srcAudioCodec 为 Format 显示名（realaudio），
    // 而 codec 字段为 CodecID（cook 流也是 cook）。匹配时两者都查。
    "cook",
    "real_144", // RA 14.4
    "real_288", // RA 28.8
    "ralf", // RealAudio Lossless
    "sipr", // RealAudio SIPR
    "realaudio", // mediainfo 的 Format 显示名（RealAudio 全家族）
    "atrac3", // ATRAC3（部分场景）
    "adpcm_ima_qt", // 无标准 mp4 tag
    "ima4", // adpcm_ima_qt 的四字符码（mediainfo CodecID 取值）
    "vorbis", // MP4 支持极差，多数构建无法封装
]

/**
 * 判断指定音频编码能否直接 copy 进目标容器
 *
 * 目前仅对 MP4 系容器做黑名单判断；其他容器（如 mkv）兼容性极好，一律返回 true。
 *
 * @param {string} audioCodec - 源音频编码名（ffprobe codec_name 或 mediainfo Format 小写，如 cook / realaudio / aac）
 * @param {string} dstExt - 目标文件扩展名（如 .mp4 / .mkv），可带或不带点
 * @param {string} [audioCodecId] - 编码 ID（ffprobe codec_tag_string 或 mediainfo CodecID），两条检测器路径取值不同，一并参与匹配
 * @returns {boolean} true 表示可以直接 `-c:a copy`，false 表示必须重编码
 */
export function isAudioCodecCompatibleWithContainer(audioCodec, dstExt, audioCodecId = "") {
    if (!audioCodec && !audioCodecId) {
        // 拿不到编码信息时保守放行，避免误伤正常流程
        return true
    }
    const codec = String(audioCodec || "").toLowerCase()
    const codecId = String(audioCodecId || "").toLowerCase()
    const ext = String(dstExt || "")
        .toLowerCase()
        .replace(/^\./, "")
    // WebM 是 Matroska 的受限子集：视频仅 VP8/VP9/AV1，音频仅 Opus/Vorbis。
    // aac/mp3/ac3 等即使能塞进 .mkv，也**不能** copy 进 .webm（会产出非法 webm）。
    if (ext === "webm") {
        const webmAudio = ["opus", "vorbis", "libopus", "libvorbis"]
        return webmAudio.includes(codec) || webmAudio.includes(codecId)
    }
    // 仅 MP4 系容器受限；MKV 等兼容性好，不做拦截
    const mp4Family = ["mp4", "m4v", "mov", "m4a"]
    if (!mp4Family.includes(ext)) {
        return true
    }
    return (
        !MP4_INCOMPATIBLE_AUDIO_CODECS.includes(codec) &&
        !MP4_INCOMPATIBLE_AUDIO_CODECS.includes(codecId)
    )
}
...
export function getFileTypeByExt(filename) {
    ...
    // RAW（.cr2/.nef/.arw/.dng…）本质是相机照片，归类为图片；
    // 注意不改 isImageFile —— 那用于 compress，RAW 无法被 sharp 解码。
    if (IMAGE_FORMATS.includes(ext) || RAW_FORMATS.includes(ext)) {
...
/**
 * 解析码率字符串为 bps（bit/s），**按 1000 进制**，与 ffmpeg `-b:v "3M"`=3_000_000 一致。
 * 区别于文件容量（FILE_SIZE_1K=1024 那套只用于字节大小，不用于码率）。
 *
 * 支持：
 *   - 纯数字  → 视为 bps（对齐 ffmpeg），如 "2000000"→2000000
 *   - 带单位  → k/m/g（大小写均可，精确后缀），如 "233k"/"233K"→233000、"3m"/"1.5M"→3_000_000/1_500_000、"2g"→2_000_000_000
 *   - 非法输入（"abc"、负数、空串）→ 抛 Error
 *
 * @param {string|number} str 码率字符串（或兼容旧传法的纯数字，视为 bps）
 * @returns {number} bps 整数
 */
export function parseBitrate(str) {
    if (typeof str === "number") {
        if (Number.isFinite(str) && str >= 0) return Math.round(str)
        throw new Error(`invalid bitrate: ${String(str)}`)
    }
    const s = String(str ?? "")
        .trim()
        .toLowerCase()
    if (s === "") throw new Error(`invalid bitrate: "${str}"`)
    const m = s.match(/^(\d+(?:\.\d+)?)\s*([kmg]?)\s*$/)
    if (!m)
        throw new Error(`invalid bitrate: "${str}" (expected e.g. "2000000", "233k", "3M", "1.5g")`)
    const num = parseFloat(m[1])
    const unit = m[2] || ""
    const mult =
        unit === "k" ? 1000 : unit === "m" ? 1000 * 1000 : unit === "g" ? 1000 * 1000 * 1000 : 1
 ...
 * Windows 保留设备名
 *
 * `CON` `PRN` `AUX` `NUL` `COM1`-`COM9` `LPT1`-`LPT9` 及其带扩展名的形式
 * （如 `CON.txt`）都无法作为普通文件创建，创建时会静默失败或打开设备。
 * 这些名字此前未被拦截。
 */
const REGEX_RESERVED_WINDOWS_NAME = /^(?:CON|PRN|AUX|NUL|COM[1-9]|LPT[1-9])(?:\.|$)/i
 ...
 */
export function isReservedWindowsName(name) {
    if (typeof name !== "string") {
        return false
    }
    return REGEX_RESERVED_WINDOWS_NAME.test(name.trim())
}

/**
 * 移除文件名中的非法字符，并规避 Windows 保留设备名
 *
 * 做了三件事：
 *   1. Unicode NFC 规范化 —— macOS/部分相机产出的是 NFD 形式的文件名
 *      （如 `é` 被拆成 `e` + U+0301），不做归一化会与 NFC 形式的同名文件
 *      被判为两个不同名字，去重与「目标已存在」判定都会失效。
 *   2. 移除非法字符（`<>:"/\|?*`、空白、控制字符）。
 *   3. 保留设备名加 `_` 前缀规避 —— 直接清空会把用户想要的名字整个丢掉。
 *
...
export function filenameSafe(name) {
    ...
    const cleaned = normalized.replaceAll(REGEX_ILLEGAL_FILENAME, "")
    return isReservedWindowsName(cleaned) ? `_${cleaned}` : cleaned
}
 ...
 * 用于存储被安全删除的文件
 *
 * @param {string} _filepath - 原始文件路径
 *
 *   保留该形参仅为兼容既有调用点（含测试）。回收目录现已固定落在用户目录下，
 *   与源文件所在磁盘无关——按盘根放置会在非管理员账户下 EACCES 失败，
 *   进而触发"删除失败却仍被计为成功"的静默数据残留。
 *   若将来要支持"回收到源文件所在盘"，可重新启用该参数。
 *
 * @returns {string} 安全删除目录的完整路径
 */
export function getSafeDeletedDir(_filepath) {
    const dtStr = dayjs().format("YYYYMMDD")
    // 回收站放在用户目录下，而不是「源文件所在磁盘的根目录」。
    // 盘根（如 C:\Deleted_By_Mediac）在非管理员账户下创建会 EACCES 失败，
    // 一旦失败就会触发"删除失败却被计为成功"的静默数据残留。
    const dir = path.join(os.homedir(), ".mediac", "deleted", dtStr)
    return path.resolve(dir)
}

/**
 * 安全删除文件，转移到回收目录而不是永久删除
 * 保留原有目录结构，防止误删重要文件
 *
 * **重要**：失败时返回 `null`，调用方必须显式判定，
 * 否则会出现"源文件还在，但计数器说已删除"的失真。
 *
 * @param {string} filepath - 要删除的文件路径
 * @returns {Promise<string|null>} 转移后的目标路径；失败时返回 null
 */
export async function safeRemove(filepath) {
    try {
        ...
        let deletedPath = path.join(deletedDir, baseName)
        // 同名冲突时递增后缀，避免只回退一层就失败（旧实现第三次碰撞会直接失败）
        for (let i = 1; await fs.pathExists(deletedPath); i++) {
            deletedPath = path.join(deletedDir, `${i}_${baseName}`)
            if (i > 9999) {
                throw new Error(`too many name conflicts in ${deletedDir}`)
            }
        }
        ...
        // 返回 null（而非 undefined）以强制调用方显式判定；
        // 错误信息同时进日志，便于事后排查"为什么文件还在"。
        log.warn(`[safeRemove] failed: ${filepath} - ${error?.message || error}`)
        return null
    }
}
...
export async function isEmptyDir(dirPath) {
    ...
    const { value } = await dirIter[Symbol.asyncIterator]().next()
    await dirIter.close()
    return !value
}
...
export function fileHash(filename, algorithm = "md5") {
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            return reject(
                new Error(`${algorithm} hash failed for <${filename}>: ${error?.message || error}`),
            )
        }
        s.on("data", function (data) {
            shasum.update(data)
        })
        // 必须监听 error：否则文件在读取过程中被移走/无权限/是目录时，
        // 既不会触发 data 也不会触发 end，Promise 永不 settle —— 调用方直接卡死。
        // （rename/dedup 这类批量场景里，文件被并发移走是常态而非异常）
        ...
        s.on("end", function () {
            try {
                ...
                reject(
                    new Error(
                        `${algorithm} digest failed for <${filename}>: ${error?.message || error}`,
                    ),
                )
            }
        })
...
export async function killProcess(procName) {
    // 注意：execa 期望「可执行文件 + 参数数组」形式，整串传入在部分平台会失败；
    // 且原实现把 /T 重复写了两次。
    switch (process.platform) {
        case "win32":
            await execa("taskkill", ["/F", "/T", "/IM", `${procName}.exe`])
            break

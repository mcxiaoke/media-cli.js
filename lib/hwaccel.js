...
export function buildVideoFilters({
    ...
    if (tier.requiresFilter && hasScale) {
...
export function buildEncoderArgs(
    tierName,
    { quality = 24, bitrateK, codecFamily = "h264", pixFmt, forcedEncoder } = {},
) {
    ...
    const familyRow = ENCODER_MATRIX[tierName] || ENCODER_MATRIX.cpu
    const encoder = forcedEncoder || familyRow[codecFamily] || ENCODER_MATRIX.cpu[codecFamily]
    const q = forcedEncoder
        ? normalizeQualityForEncoder(forcedEncoder, codecFamily, quality, pixFmt)
        : normalizeQuality(tierName, codecFamily, quality, pixFmt)
...
export async function selectTier({
    ...
    if (strict && decodeMode !== DecodeMode.CPU) {
        const hwTiers = tiers.filter((t) => t.name !== "cpu")
        if (hwTiers.length === 0) {
            throw new Error(

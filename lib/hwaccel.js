 ...
 *   qsv    : -cq {q}        ❌ **被静默忽略**（NVENC 专属参数，qsv 三档产物完全相同）
 *            -global_quality {q}  ✅ 生效（ICQ 智能恒定质量，官方文档推荐的 quality-based 码控）
 *            -q:v {q}        ✅ 生效（qscale flag → CQP 固定 QP，遗留写法；10bit 源低值会失效）
 *   libx264: -crf {q}       ✅ 生效
 ...
 */
export function probeCacheKey({ tierName, codec, pixFmt, codecFamily, dimension, bitDepth }) {
    return `${tierName}|${codec}|${codecFamily}|${pixFmt}|${dimension}|${bitDepth || ""}`
}

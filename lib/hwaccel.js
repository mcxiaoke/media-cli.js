...
export function buildEncoderArgs(
    ...
    switch (impl) {
        case "nvenc":
            ...
            // 调优参数：spatial_aq/temporal_aq 开启自适应量化，rc_lookahead 提升码控精度
            // surfaces 保证 4K 编码器缓冲区，weighted_pred 改善渐变场景，b_ref_mode 提升B帧效率
            args.push("-rc", "vbr", "-tune", "hq", "-cq", String(q))
            args.push("-spatial_aq", "1", "-temporal_aq", "1")
            args.push("-rc_lookahead", "32", "-surfaces", "64")
            args.push("-weighted_pred", "1", "-b_ref_mode", "each")
        ...
        case "qsv":
            ...
            // 调优参数：look_ahead + extbrc 提升码控质量，async_depth 保证编码器流水线深度
            args.push("-global_quality", String(q))
            args.push("-look_ahead", "1", "-look_ahead_depth", "100")
            args.push("-async_depth", "4", "-extbrc", "1")
            if (bitrateK) args.push("-b:v", bitrateK, "-maxrate", bitrateK)
            break
        case "amf":
            // amf：qp 系列（本机无 A 卡，未实测；参数依据 docs/ffmpeg/ffmpeg-guide-encoders.md）
            // 调优参数：quality 设为 quality 保证画面质量，header_insertion_mode 优化 SPS/PPS 插入策略
            args.push("-qp_i", String(q), "-qp_p", String(q))
            args.push("-quality", "quality", "-header_insertion_mode", "gop")
            if (bitrateK) args.push("-b:v", bitrateK)
            break
        case "cpu":
        default:
            // CPU 编码器（libx264 / libx265 / libvpx-vp9 / libaom-av1 / libsvtav1）
            // 调优参数：-preset 控制编码速度与压缩效率平衡，-tune 针对内容类型优化

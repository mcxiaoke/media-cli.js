...
export function buildEncoderArgs(
    ...
    switch (impl) {
        case "nvenc": {
            // NVIDIA 官方推荐 VBR；纯质量用 -cq（配 -b:v 0 关闭码率上限）。
            args.push("-rc", "vbr", "-tune", "hq")
            if (bitrateK) {
                args.push("-b:v", bitrateK, "-maxrate", bitrateK, "-bufsize", bitrateK)
            } else {
                args.push("-cq", String(q), "-b:v", "0")
            }
            // 已删（可选调优，易崩/收益不确定）：-spatial-aq -temporal-aq -rc-lookahead
            //   -surfaces -b_ref_mode -weighted_pred。
            break
        }
        case "qsv": {
            // Intel 官方 ICQ 质量模式：-global_quality（实测有效）。
            if (bitrateK) {
                args.push("-b:v", bitrateK, "-maxrate", bitrateK)
            } else {
                args.push("-global_quality", String(q))
            }
            // 已删（本机 qsv 不在候选链、未经验证的调优项）：
            //   -look_ahead -look_ahead_depth -async_depth -extbrc。
            break
        }
        case "amf": {
            // ⚠️ 本机无 A 卡，amf 选项拼写/取值无法验证。为避免同类 "Unrecognized option"
            //   崩溃，这里只保留最保守的码率控制；预设质量值暂不下发到 amf，待 A 卡实测再补。
        ...
        default: {
            // CPU 编码器：CRF 质量。
            //  x264/x265：-crf + -preset，可叠加 -maxrate/-bufsize(VBV 上限)。
            //  libaom-av1 / libsvtav1 / libvpx-vp9：纯 CRF 必须 -b:v 0，且**不能**带
            //    -maxrate/-bufsize（否则报 "Rate control parameters set without a bitrate"）。

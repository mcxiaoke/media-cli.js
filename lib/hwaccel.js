...
export function buildEncoderArgs(
    ...
    // kb() 格式化为 "xxxK"。bitrate 是预设目标码率、已按分辨率 scale 过，故峰值上限
    // 直接取 bitrate×1.5（无需独立字段、也已随分辨率缩放）。带 K 的模板字段
    // （videoBitrateK/audioBitrateK）是**字符串**，仅供文件名/模板（audioArgs、suffix）
    // 注入使用，不参与这里的码率计算。
    const kb = (v) => `${Math.round((v || 0) / 1000)}K`
    const b = bitrate || 0
    const m = Math.round(b * 1.5)
    const usingVbr = b > 0
    ...
    // 统一按「有无目标码率」分发两种码控模式：
    //   bitrate(bps) > 0 → VBR（目标码率）模式：-b:v = bitrate，-maxrate = bitrate×1.5；
    ...
    switch (impl) {
        case "nvenc": {
            ...
            // 可选（不加默认）：-spatial-aq / -temporal-aq / -b_ref_mode（连字符拼写）——
            //   有效但属内容/强度取向，不宜写死默认。
            break
        }
        case "qsv": {
        ...
        default: {
            ...
            if (codecFamily === "h264" || codecFamily === "hevc") {
                if (usingVbr) {
                    args.push("-b:v", kb(b), "-preset", "medium", "-maxrate", kb(m), "-bufsize", kb(m))
                } else {

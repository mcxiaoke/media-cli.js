/*
 * File: cue-parse.js
 * CUE文件解析器 - 解析CUE sheet文件获取音频分割信息
...
class CueParser {
    ...
    async parse() {
        try {
            ...
            log.error("Error parsing CUE file:", error)
            return null
        }
    }
}
...
// 注意：此处曾有一行顶层 `await testParse()`，会导致 import 该模块即崩溃
// （testParse 内部引用了不存在的函数）。调试代码不应留在模块顶层，
// 已移除；如需手动验证解析逻辑，请显式调用 testParse()。

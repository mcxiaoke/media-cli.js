                ...
                switch (keyword) {
                    ...
                    case 'REM':
                        ...
                        if (remParts && remParts.length == 2) {
                            if (currentTrack) {
                                currentTrack.REM.set(removeQuotes(remParts[0]), removeQuotes(remParts[1]))
                            } else {
                                this.cueInfo.REM.set(removeQuotes(remParts[0]), removeQuotes(remParts[1]))
                            }
                        }
                        break
                    case 'CATALOG':
                    ...
                    case 'TRACK':
                        ...
                        currentTrack = { TRACK_NO: trackNo, TYPE: audioType, START: 0, END: 0, START_STR: '', END_STR: '', REM: new Map(), TITLE: this.currentTrackInfo.TITLE, PERFORMER: this.currentTrackInfo.PERFORMER }
...
function testMergeMapToObject() {
    ...
    // 覆盖原有字段
    const mergedObject2 = mergeMapToObject(map, {
        key2: 'originalValue',
        key4: 'originalValue'
    }, true)

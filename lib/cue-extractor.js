/*
 * File: cue-extractor.js
 * CUE文件提取器 - 从CUE文件中解析音频文件并批量提取音轨
...
if (!fs.existsSync(inputDir)) {
    log.showRed("Input directory does not exist.")
    process.exit(1)
}
...
if (cueFiles.length === 0) {
    log.showRed("No cue files found in the input directory.")
    process.exit(1)
}

log.show(`Found ${cueFiles.length} cue file(s) in the input directory.`)
log.show("Splitting audio by cue files...")
...
;(async () => {
    for (const cueFile of cueFiles) {
        ...
        log.show(`Splitting audio for cue file: ${cueFile}`)
        ...
        try {
            ...
            log.show(`Audio split successfully for cue file: ${cueFile}`)
            log.debug(cueInfo)
        } catch (error) {
            log.showRed(`Error splitting audio for cue file ${cueFile}: ${error.message}`)
        }
    }
})()

...
import hanziRarelyUsedData from "../assets/hanzi/hanzi_rarely.json" with { type: "json" }
import hanziComplexData from "../assets/hanzi/hanzi_complex.json" with { type: "json" }
import hanziCommon3500Data from "../assets/hanzi/hanzi_common_3500.json" with { type: "json" }
import hanziCommon7000Data from "../assets/hanzi/hanzi_common_7000.json" with { type: "json" }
import hanziCommonJapaneseData from "../assets/hanzi/hanzi_common_japanese.json" with { type: "json" }
...
export const JAPANESE_KANA =
    "ぁあぃいぅうぇえぉおかがきぎくぐけげこごさざしじすずせぜそぞただちぢっつづてでとどなにぬねのはばぱひびぴふぶぷへべぺほぼぽまみむめもゃやゅゆょよらりるれろゎわゐゑをんゔゕゖゝゞゟァアィイゥウェエォオカガキギクグケゲコゴサザシジスズセゼソゾタダチヂッツヅテデトドナニヌネノハバパヒビピフブプヘベペホボポマミムメモャヤュユョヨラリルレロヮワヰヱヲンヴヵヶヷヸヹヺーヽヾヿ"

//-------------------------------------------------------------------------------

// 导出一个正则表达式，用于判断字符串中是否只包含ASCII字符
export const REGEX_ASCII_ONLY = /^[\x20-\x7F]+$/ // or es2018: /^[\p{ASCII}]+$/u
/**
 * 判断给定字符串是否只包含ASCII字符
...
export const REGEX_ASCII_ANY = /[\x20-\x7F]/
/**
 ...
 */
export const REGEX_JAPANESE_HAN = new RegExp(
    `^[${HANZI_COMMON_JAPANESE}A-Za-z0-9_.
-]+$`,
    "u",
)
...
export const REGEX_CHINESE_ANY =
    /[\u3400-\u4dbf]|[\u3300-\u33ff]|[\u4e00-\u9fff]|[\uf900-\ufaff]|[\ufe30-\ufe4f]/u
// 注意：必须用单个字符类 + 首尾锚点表达"仅由中文组成"。
// 此前写作 /^[A]|[B]|[C]|[D]|[E]+$/u，被解析为 (^A)|(B)|(C)|(D)|(E+$)，
// 导致任何包含 B/C/D 的字符串（如 "a|b"）都被误判为"纯中文"。
...
// 注意：模板串中不要出现字面换行，且 Unicode 转义应为 \uXXXX
// 而非 \\uXXXX（后者是「字面反斜杠 + u」，会与相邻的 . 组成
// 范围 .-\，把 / 等字符误判为中文）。
export const REGEX_CHINESE_ONLY = new RegExp(
    `^[ A-Za-z0-9_.\u00a1-\u00b7\u2012-\u2051\u2150-\u218f\u2600-\u27bf\u3000-\u303f\u3040-\u309f\u30a0-\u30ff\uff01-\uff20${HANZI_COMMON_3500}]+$`,
    "u",
)
...
export const NON_CJKERA_CHARS =
    /[^\p{sc=Hani}\p{sc=Hira}\p{sc=Kana}\p{sc=Hang}\p{sc=Cyrl}A-Za-z0-9 _\-.]/u

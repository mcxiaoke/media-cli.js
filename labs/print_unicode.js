...
function sortByStroke() {
    ...
    // 复杂汉字
    // 不包含在中日常用汉字里
    let data = strokeData.filter(
        (e) =>
            !(
                unicode.HANZI_COMMON_7000.includes(e.key) ||
                unicode.HANZI_COMMON_JAPANESE.includes(e.key)
            ),
    )
    // 笔画大于等于18的汉字
    ...
    // 生僻汉字
    // 不包含在中日常用汉字里
    let rarelyUsedChars = Array.from(enc.MESSY_CJK_CHARS).filter(
        (c) =>
            !(unicode.HANZI_COMMON_7000.includes(c) || unicode.HANZI_COMMON_JAPANESE.includes(c)),
    )
    // 不包含在复杂汉字里

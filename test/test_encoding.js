...
assert.strictEqual(
    enc.charUnique("测试测试"),
    "测试",
    "charUnique should remove duplicate Chinese characters",
)
...
assert.strictEqual(
    noBadUnicodeResult.length,
    0,
    "checkBadUnicode should not detect bad Unicode in normal string",
)
...
assert.strictEqual(
    enc.hasBadUnicode("hello?"),
    true,
    "hasBadUnicode should return true for string with bad Unicode",
)
assert.strictEqual(
    enc.hasBadUnicode("hello"),
    false,
    "hasBadUnicode should return false for string without bad Unicode",
)
assert.strictEqual(
    enc.hasBadUnicode(""),
    false,
    "hasBadUnicode should return false for empty string",
)
assert.strictEqual(
    enc.hasBadUnicode(123),
    false,
    "hasBadUnicode should return false for non-string input",
)
...
assert.strictEqual(
    enc.hasBadCJKChar("hello"),
    false,
    "hasBadCJKChar should return false for string without bad CJK characters",
)
assert.strictEqual(
    enc.hasBadCJKChar(""),
    false,
    "hasBadCJKChar should return false for empty string",
)
assert.strictEqual(
    enc.hasBadCJKChar(123),
    false,
    "hasBadCJKChar should return false for non-string input",
)
...
assert.ok(
    Array.isArray(nonArrayOrder),
    "getOptimizedEncodingOrder should handle non-array encoding inputs",
)
